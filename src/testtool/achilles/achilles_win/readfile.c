 ...
 * History:
 *	26-Sept-89 (GordonW)
 *		added childnum for setting an optional number of children.
 *	27-Sept-89 (GordonW)
 *		use a portable version of MEreqmem.
 *	05-July-91 (DonJ)
 *		Change quotes to brackets on include of achilles.h.
 *	24-January-96 (bocpa01)
 *		Converted to support NT
**      22-sep-2000 (mcgem01)
...
GLOBALREF	int		 n_dbname;
GLOBALREF	int		 n_forks;
u_i2	*CM_AttrTab;
...
TESTDESC ***testarr;
{
	...
	while ( (c = SIgetc(in) ) != EOF)
	{
	    /* Found a comment. Discard to end of line, leaving the newline
	       so the next SIgetc() will take care of end-of-line
	       processing.
	    */
	    if (c == '#')
	    {
	    ...
	    */
		...
		switch (state)
		{
		    ...
		    case ST_NKIDS:
			/* If argument is <MAXKIDS>, we'll copy the data-
			   base name rather than the argument.
			*/
			rval = STcompare(tempdata, "<MAXKIDS>");
			if ( (!rval) && (!maxkids) )
			{
			    SIprintf("Line %d, %s: executable %s requires maxium number of children, but none set.\n",
				      linenum, fname, curtest->t_argv[0]);
			    errcount++;
			}
			else
			    gooddata = (rval ? tempdata : maxkids);
			CVan(gooddata, &curtest->t_nkids);
			break;
		...
		*/
		...
		{
		    /* If >1 consecutive whitespace character, skip over the
		       rest.
		    */
		    while (1) {
			c = SIgetc(in);
		    ...
		    */
		    if (++state == ST_INVALID)
		    {
			printf("Line %d, %s: too many fields on a line.\n",
			  linenum, fname);
			errcount++;
			SIungetc('#', in);
			state = ST_NEWLINE;
	    ...
	    */
	    ...
	    {
		testcount++;
		newtest = (TESTDESC *) MEreqmem(0, sizeof(TESTDESC), TRUE, &status);
		if( status != OK )
		{
			SIprintf("Can't MEreqmem TESTDESC\n");
			return(-1);
		}
		newtest->t_next = NULL;
		if (curtest)
	...
	n_forks = (curtest->t_nkids * curtest->t_niters);
	n_dbname = dbcnt;
	...
	{
	    /* Make an array of pointers, and assign them to the test
	       structures.
	    */
	    *testarr = (TESTDESC **)
		MEreqmem(0, testcount*sizeof(TESTDESC *), TRUE, &status);
	    if( status != OK )
	    {
		SIprintf("Can't MEreqmem TESTDESC\n");
		return(-1);
