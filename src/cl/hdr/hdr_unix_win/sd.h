...
**      06-feb-96 (johnst)
**	    Bug 72205.
**	    Adding true binary compression (TCB_C_HICOMPRESS) to ingres broke 
**	    ICL SCAFS which only knew the previous TCB_C_STANDARD compression.
**	    Changed SD_ATTR_INFO.compressed element from a bool to a char
**	    and defined SD_C_NONE, SD_C_STANDARD, SD_C_HICOMPRESS ingres
**	    storage compression types to pass no/low/hi compression info
**	    through this interface. The compression type values are copied
**	    from the TCB_C_* values defined in dmp.h.
**	21-jan-1999 (hanch04)
