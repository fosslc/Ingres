...
**	    is MUTEX_MASK.
**	30-jul-1997 (teresak)
**	    Added defined for exhdef.h for the OpenVMS exit handler. This file
**	    must be included before csinternal.h.
**      06-aug-1997 (teresak)
**          removed call to routine CS_realtime_update_smclock. As work 
**	    progresses on threads, it may be necessary implement a version of 
**	    this routine on VMS.
**	19-jul-2000 (kinte01)
  ...
  */
  ...
  for (;;)
  {
	...
	if (an_scb->cs_thread_type >= -1 && 
	    an_scb->cs_thread_type <= MAXTHREADS - 1)
	    cs_thread_type = an_scb->cs_thread_type;
	else
	    cs_thread_type = MAXTHREADS - 1;	/* use the <invalid> type */
