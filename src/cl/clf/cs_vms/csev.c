...
**  17-jun-2009 (stegr01)
**     add support for Posix threads on Itanium so this will function with CSMT
**     The #ifdef VMS stuff is in here because it was originally 
**     envisaged that the VMS source would simply include the Unix
**     source - since this is where it came from.
...
PTR		CSev_base;
u_i4		CSservernum;
GLOBALDEF i4    CSslavepids[NSVSEMS] ZERO_FILL;
# ifdef JASMINE
...
# endif
	...
	if ( 0 != (rad = CXnuma_cluster_rad()) )
	    ...
	    if (str && *str)
	    {
            ...
            CSslavepids[j] = 0;
#else
        CSslavepids[j] = PCfork(&status);
#endif
            switch (CSslavepids[j])
	    {
	    case 0:	/* in child */
	        close(0);
	...
	*/
	...
	for (i = css_wakeup->css_maxused; i >= 0; i--)
	{
		...
		if (wakeup_blocks[i].inuse && Cs_srv_block.cs_mt)
		{
            ...
            wakeup_blocks[i].astqueue[0] = wakeup_blocks[i].astqueue[1] = 0;
		}
...
CSMT_get_wakeup_block(CS_SCB *scb)
{
    ...
    /* initialise the self-relative queue header */

    scb->cs_evcb->astqueue[0] = scb->cs_evcb->astqueue[1] = 0;
...
CS_SCB 		*scb)
{
	...
	if (wakeup_block->inuse == 1)
	{
        ...
        wakeup_block->astqueue[0] = wakeup_block->astqueue[1] = 0;
...
CS_clear_wakeup_block(PID pid, CS_SID sid)
{
    ...
    static int dummy = 0; /* To avoid compilation warnings */
