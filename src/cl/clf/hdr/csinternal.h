...
#ifndef CSINTERNAL_H_INCLUDED
#define CSINTERNAL_H_INCLUDED
...
** Description:
**
**	This header contains CS (Control System) definitions that are
**	global to all flavors of CS and CSMT, and perhaps even known to
**	other parts of the CL.  These are definitions that are *not*
**	exported outside of the CL.  Nor are they strictly private to
**	a particular CS implementation.
**
** History: $Log-for RCS$
**      29-Oct-1986 (fred)
**          Created.
...
**	    Merge vms and unix+win csinternal's, they were almost identical
**	    anyway.  Move to clf/hdr.
**/

/*
...
**
** This definition doesn't apply to VMS.
*/
...
**
**	* Note * If you change any structure member offsets, and in particular
**		cs_current
**		cs_inkernel
**	Then you will have to fix all of the internal-threads ASM files.  I've
**	moved them to (near) the front to minimize the chances that they will
**	ever move again.  (Of course, now that we hardly ever use the
**	ASM internal thread switchers any more!)
**
** History:
**      27-Oct-1986 (fred)
**          Initial Definition.
...
struct _CS_SYSTEM
{
    ...
    ** DO NOT MAKE CHANGES IN THIS STRUCTURE from here to 'cs_current'
...
#define                 CS_MARKER       "CS_SYSTEM BLOCK"
    i4		    cs_inkernel;	/* (0x10) Are we interruptible */
    CS_SCB	    *cs_current;	/* (0x14) The currently active session */

    STATUS          (*cs_scballoc)(CS_SCB **, void *, i4);
					/* Routine to get SCB's from */
    STATUS          (*cs_scbdealloc)(CS_SCB *); /* Routine thru which to give them back */
    void	    (*cs_elog)(i4, CL_ERR_DESC *, i4, ...);
					/* Routine with which to log errors */
    STATUS          (*cs_process)(i4, CS_SCB *, i4 *); /* General processing of stuff ... */
    STATUS          (*cs_attn)(i4, CS_SCB *); /* Processing of async stuff ... */
    STATUS          (*cs_startup)(CS_INFO_CB *);  /* To start up the server */
    STATUS          (*cs_shutdown)(void);   /* To shut the server down */
    STATUS	    (*cs_format)(CS_SCB *,char *,i4,i4); /* format scb's */
    STATUS	    (*cs_saddr)(void *, i4); /* To get session requests */
    STATUS	    (*cs_reject)(void *, STATUS); /* To reject session requests */
    VOID	    (*cs_disconnect)(CS_SCB *); /* To disconnect sessions */
    STATUS	    (*cs_read)(CS_SCB *, i4);  /* To read from FE's */
    STATUS	    (*cs_write)(CS_SCB *, i4); /* To write to FE's */
...
# endif /* OS_THREADS_USED */
    ...
    STATUS	(*cs_diag)(void *);	/* Diagnostics */
    ...
    char *	(*cs_format_lkkey)(struct _LK_LOCK_KEY *, char *);/* Fmt LK_LOCK_KEY for display. */
...
# ifdef OS_THREADS_USED
    ...
    PID		(*cs_get_rcp_pid)(void); /* To realize RCP's pid */
...
FUNC_EXTERN void CS_dump_stack(CS_SCB *, void *, PTR, TR_OUTPUT_FCN *, i4);
FUNC_EXTERN void CS_fmt_scb(CS_SCB *, i4, char *);
...
FUNC_EXTERN STATUS CSMT_get_thread_stats(CS_THREAD_STATS_CB *);
FUNC_EXTERN void CSMT_swuser(void);
#endif

/* CS things that are unix+vms, but not Windows */
#ifndef NT_GENERIC
FUNC_EXTERN STATUS CS_alloc_stack(CS_SCB *, CL_ERR_DESC *);
FUNC_EXTERN u_i2 CS_compress(time_t);
FUNC_EXTERN STATUS CS_deal_stack(CS_STK_CB *);
FUNC_EXTERN i4 CS_hwm_active(void);
FUNC_EXTERN void CS_move_async(CS_SCB *);
FUNC_EXTERN STATUS CS_null_handler(EX_ARGS *);
FUNC_EXTERN i4 CS_num_active(void);
FUNC_EXTERN void CS_recusage(void);
FUNC_EXTERN CS_SCB * CS_verify_sid(char *, char *, TR_OUTPUT_FCN *, i4);
FUNC_EXTERN i4 CSmon_try_registered_diags(TR_OUTPUT_FCN *, char *);
FUNC_EXTERN void CSprofstart(void);
FUNC_EXTERN u_i4 CSsamp_elf_hash(const u_char *name);
FUNC_EXTERN void CSsigterm(i4);
#endif

/* Data references needed by CS or CSMT */

/* VMS wants these two volatile, whether for valid reason who knows.
** (A better way would be to mark individual bits volatile as needed.)
** Volatile on unix/windows causes all sorts of "discards pointer
** qualification" messages from pthread calls and elsewhere.
*/

#ifdef VMS
GLOBALREF volatile CS_SYSTEM Cs_srv_block;	/* CS server control block */
GLOBALREF volatile CS_ADMIN_SCB Cs_admin_scb;	/* Admin thread session CB */

#else

GLOBALREF CS_SYSTEM Cs_srv_block;	/* CS server control block */
GLOBALREF CS_ADMIN_SCB Cs_admin_scb;	/* Admin thread session CB */
