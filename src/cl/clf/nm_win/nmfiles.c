...
**  11-Feb-2009 (drivi01)
**	Redefine the way II_SYSTEM is retrieved.  Add routines to use currently
**      loaded DLL to figure out II_SYSTEM and set the environment.
**  06-May-2009 (whiro01)
**	Fix the new II_SYSTEM code to punt to the old method if the DLL path
**	doesn't end with .../ingres/bin, .../ingres/utility or .../ingres/lib.
**	Also eliminated all compiler warnings under Windows.
**/
...
static char	iisystem[MAX_LOC + 1]={'\0'};
static BOOL (FAR WINAPI *pfnGetModuleHandleEx)(
			DWORD dwFlags,
...
# endif
	    STpolycat( 5, NM_static.Sysbuf, SLASHSTR, "admin", SLASHSTR, buf, NM_static.Admbuf );
	    LOfroms( PATH, NM_static.Admbuf, &NM_static.Admloc );
	...
	if( !found )
	{
	    STpolycat( 3, NM_static.Sysbuf, SLASHSTR, "files", NM_static.Admbuf );
	    LOfroms( PATH, NM_static.Admbuf, &NM_static.Admloc );
	}
...
/*
**  Name: SetOneEnvVar(char *, char *)
**
**  Description:
**
**  History:
**	06-Feb-2009 (drivi01)
**	    Reuse the function for setting the environment.
*/

void SetOneEnvVar(char *lpEnv,char *addValue)
{
	char temp='\0';
	int nret;
	char *oldEnv=NULL;
	...
	oldEnv=(char *)MEreqmem(0, nret+1, FALSE, NULL);
	if (oldEnv)
		GetEnvironmentVariable(lpEnv, oldEnv, nret+1);
	buf=(char *)MEreqmem(0, nret+STlength(addValue)+1, FALSE, NULL);
...
**	    Reuse the function for setting the environment.
**	09-Mar-2009 (drivi01)
**	    Insert double slash between II_SYSTEM and
**	    SystemLocationVariable before setting
**	    path, lib and include paths. Replace "\\" with
**	    SLASHSTR.
*/

void SetEnvVars(char *lpValue)
{
	char path_buf[MAX_PATH];
	char buf[MAX_PATH];

	SetEnvironmentVariable(SystemLocationVariable,NULL);
	SetEnvironmentVariable(SystemLocationVariable,lpValue);

	STpolycat(3, lpValue, SLASHSTR, SYSTEM_LOCATION_SUBDIRECTORY,buf);
	STprintf(path_buf,"%s%sbin;%s%sutility",buf, SLASHSTR, buf, SLASHSTR);
	SetOneEnvVar("PATH", path_buf);

	STprintf(path_buf, "%s%slib", buf, SLASHSTR);
	SetOneEnvVar("LIB",path_buf);

	STprintf(path_buf,"%s%sfiles", buf, SLASHSTR);
	SetOneEnvVar("INCLUDE",path_buf);
}


/*{
** Name: NMgtEnv - retrieve II_SYSTEM variable and set the environment.
**
**
** Inputs:
**	None
...
**	    Created.
**	09-Mar-2009 (drivi01)
**	    Remove the last slash in II_SYSTEM before
**	    setting the environment. Replace "\\" with
**	    SLASHSTR.
**	08-Apr-2009 (drivi01)
**          Fix the code to avoid calling GetModuleHandleEx directly
**          as the function doesn't exist in the kernel32.dll on
**          Windows 2000.
**	06-May-2009 (whiro01)
**	    Fix the new II_SYSTEM code to punt to the old method if the DLL path
**	    doesn't end with .../ingres/bin, .../ingres/utility or .../ingres/lib.
...
NMgtEnv()
{
	...
	{
	    pfnGetModuleHandleEx = (BOOL (FAR WINAPI *)(DWORD dwFlags,
			LPCTSTR lpModuleName,
			HMODULE *phModule))
			GetProcAddress(hDll, TEXT("GetModuleHandleExA"));
	}
	if (pfnGetModuleHandleEx != NULL)
	{
	    if ((*pfnGetModuleHandleEx)(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, pEnv, &hModule))
	    {
		dwSize=sizeof(envbuffer);
		if (GetModuleFileName(hModule, envbuffer, dwSize) != 0)
		{
		    if(*envbuffer)
		    {
			LOCATION loc;
			char drive[MAX_LOC], dir[MAX_LOC], fname[MAX_LOC], ext[MAX_LOC], version[MAX_LOC];
			char *ptr, *p1, *p2;

			if (LOfroms(FILENAME & PATH, envbuffer, &loc) == OK &&
			    LOdetail(&loc, drive, dir, fname, ext, version) == OK)
			{
			    if ((ptr = STrchr(dir, SLASH)) != NULL)
			    {
				*ptr = EOS;
				p2 = ptr+1;
				if ((ptr = STrchr(dir, SLASH)) != NULL)
				{
				    *ptr = EOS;
				    p1 = ptr+1;
				    if (STcasecmp(p1, SYSTEM_LOCATION_SUBDIRECTORY) == 0)
				    {
					if (STcasecmp(p2, "bin") == 0 ||
					    STcasecmp(p2, "lib") == 0 ||
					    STcasecmp(p2, "utility") == 0)
					{
					    STpolycat(3, drive, ":", dir, iisystem);
					    SetEnvVars(iisystem);
					}
				    }
				}
