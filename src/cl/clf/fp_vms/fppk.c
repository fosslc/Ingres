...
**
**          FPpkadd() - Add two packed decimals via software
**          FPpksub() - Subtract two packed decimals via software
**          FPpkmul() - Multiply two packed decimals via software
...

/*{
** Name: FPpkadd() - Add two packed decimals.
**
** Description:
...
** History:
**      12-jan-1993 (rog)
**          Created from MHpkadd().
**	14-feb-2007 (dougi)
**	    Replaced hard constant 31 with CL_MAX_DECPREC.
*/
 
STATUS
FPpkadd( PTR pk1, i4  p1, i4  s1, PTR pk2, i4  p2, i4  s2, PTR pkout,
	 i4  *pout, i4  *sout )
{
	...
	u_char	pktmp[(CL_MAX_DECPREC+1)/2];
	    ...
	    return ( FPpksub(pk2, p2, s2, (PTR) pktmp, p1, s1, pkout,
			     pout, sout) );
        }
        else
        {
	    MEcopy(pk2, (p2 / 2 + 1), (PTR)pktmp);
            ...
            return ( FPpksub(pk1, p1, s1, (PTR) pktmp, p2, s2, pkout,
			     pout, sout) );
        }
    }
...

/*{
** Name: FPpksub() - Subtract two packed decimals.
**
** Description:
...
STATUS
FPpksub( PTR pk1, i4  p1, i4  s1, PTR pk2, i4  p2, i4  s2, PTR pkout,
	 i4  *pout, i4  *sout )
{
	...
	u_char	pktmp[(CL_MAX_DECPREC+1)/2];
	    ...
	    MEcopy(pk2, (p2 / 2 + 1), (PTR)pktmp);
            ...
            return ( FPpkadd(pk1, p1, s1, (PTR) pktmp, p2, s2, pkout,
			     pout, sout) );
        }
        else
        {
...

/*{
** Name: FPpkmul() - Multiply two packed decimals.
**
** Description:
...
** History:
**      12-jan-1993 (rog)
**          Created from MHpkmul().
**	14-feb-2007 (dougi)
**	    Replaced hard constant 31 with CL_MAX_DECPREC.
*/

STATUS
FPpkmul( PTR pk1, i4  p1, i4  s1, PTR pk2, i4  p2, i4  s2, PTR pkout,
	 i4  *pout, i4  *sout )
{
...
** History:
**      12-jan-1993 (rog)
**          Created from MHpkdiv().
**	14-feb-2007 (dougi)
**	    Replaced hard constant 31 with CL_MAX_DECPREC.
*/
 
STATUS
FPpkdiv( PTR pk1, i4  p1, i4  s1, PTR pk2, i4  p2, i4  s2, PTR pkout,
	 i4  *pout, i4  *sout )
{
    ...
    *pout = CL_MAX_DECPREC;
    *sout = max(0, (CL_MAX_DECPREC - p1 + s1 - s2));
    pk1frac = *sout + s2;
...

/*{
** Name: FPpkcmp() - Compare two packed decimals.
**
** Description:
...
** History:
**      12-jan-1993 (rog)
**          Created from MHpkcmp().
*/
 
i4
FPpkcmp( PTR pk1, i4  p1, i4  s1, PTR pk2, i4  p2, i4  s2 )
{
    ...
    neg1 = (   (ptr1[p1 / 2] & 0xf) == FP_PK_MINUS
            || (ptr1[p1 / 2] & 0xf) == FP_PK_AMINUS);
    neg2 = (   (ptr2[p2 / 2] & 0xf) == FP_PK_MINUS
	    || (ptr2[p2 / 2] & 0xf) == FP_PK_AMINUS);
