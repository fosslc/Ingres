...
!	ld	[%l1 + CS_INKERNEL],%l0
	sethi	%hh(Cs_srv_block + CS_INKERNEL),%l4
	sethi	%lm(Cs_srv_block + CS_INKERNEL),%l1
	or	%l4,%hm(Cs_srv_block + CS_INKERNEL),%l4
	or	%l1,%lo(Cs_srv_block + CS_INKERNEL),%l1
	sllx	%l4,32,%l4
	or	%l1,%l4,%l1
	ld	[%l1],%l0
...
!        ld      [%l1 + CS_CURRENT],%l0
	...
	sethi	%hh(Cs_srv_block + CS_CURRENT),%l4
	sethi	%lm(Cs_srv_block + CS_CURRENT),%l1
	or	%l4,%hm(Cs_srv_block + CS_CURRENT),%l4
	or	%l1,%lo(Cs_srv_block + CS_CURRENT),%l1
	sllx	%l4,32,%l4
	or	%l1,%l4,%l1
	ld	[%l1],%l0
...
!	ld	[%l1 + %lo(ex_sptr)], %l2
	...
	st	%i6, [%l0 + CS_PC]	!store return address of caller
!
...
!
	...
	ldd	[%o0 + CS_SP], %fp	!restore stack pointer of new thread

	ldd	[%o0 + CS_PC], %i6	!restore return address of new thread

	ldd	[%o0 + CS_EXSP], %l0	!restore EX stack ptr of new thread
!	sethi	%hi(ex_sptr), %l1
!	st	%l0, [%l1 + %lo(ex_sptr)]
	sethi	%hh(ex_sptr),%l4
	sethi	%lm(ex_sptr),%l1
...
!{
! Name: CScasptr()        - Atomically Compare & Swap 4 byte value.
!
! Description:
!      This routine performs an atomic compare and swap of an
!      aligned four byte value.   Logically this behaves like the
...
!      the target address have changed.
!
! STATUS CScasptr( PTR *addr, PTR oldval, PTR newval )
! {
!     if ( *addr == oldval )
!     {
!        *addr = newval;
