...
##  --  Master-only in simple fields UPDATE frame; template file 'msupdate.tf'
##INCLUDE indoc.tf
##INCLUDE intopdef.tf
INITIALIZE (
...
##  GENERATE QUERY SELECT MASTER REPEATED NOTERM
...
##
##  INCLUDE inmssvmn.tf         -- Save menuitem.
##
##  IF $insert_master_allowed THEN
...
##  INCLUDE inlookup.tf         -- ListChoices menuitem.
    ...
    BEGIN
##      IF $singleton_select THEN
##      GENERATE HELP fgmsunnp.hlp
##      ELSE
##      GENERATE HELP fgmsuxnp.hlp
##      ENDIF
    END
##
...
##  GENERATE USER_ESCAPE QUERY_END
    ...
    ** last selected master; TopFrame or End menuitem; Delete the
...
##INCLUDE inmsanmn.tf           -- Menuitems for Appending new data.
##
##ELSE          -- add "dummy" End menuitem
##
...
##ELSE          -- Query with attached menu (!= $singleton_select)
##
##  GENERATE QUERY SELECT MASTER NOTERM
...
##      IF ('$locks_held' = 'none' OR '$locks_held' = 'optimistic') THEN
        COMMIT WORK;                    /* release locks */
##      ENDIF
        /* submenu temporarily puts form in update mode */
##      IF $insert_master_allowed THEN
        IIsaveasnew = 'n';    /* Set status variable. 'n' means the AddNew
                              ** menuitem has not been run on this data.
                              */
...
##  INCLUDE inlookup.tf         -- ListChoices menuitem.
    ...
    BEGIN
##      IF $singleton_select THEN
##      GENERATE HELP fgmsunqp.hlp
##      ELSE
##      GENERATE HELP fgmsuxqp.hlp
##      ENDIF
    END
...
##DEFINE $_loopa 'IIloop7'
##DEFINE $_loopb 'IIloop8'
##INCLUDE inmsammn.tf           -- AppendMode menuitem.
##
##ENDIF    -- $insert_master_allowed
