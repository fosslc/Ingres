...
# include       <erug.h>
/*
** Name: IIraat_record_replace - replace record using the RAAT API
**
** Description:
**	Formulate GCA message for RAAT record replace command and send it to
...
*/
STATUS
IIraat_record_replace (RAAT_CB	*raat_cb)
{
    ...
    IIraat_trace(RAAT_RECORD_REPLACE, raat_cb);
    ...
    */
    ...
    /*
    ** Fill out gca data area, for record replace the format is:
    **
    ** i4		op_code
    ...
    *((i4 *)buf_ptr) = RAAT_RECORD_REPLACE;
    msg_size = sizeof(i4);
	...
	** copy data value (recnum) to gca buffer and increment 
    	** message size
    	*/
    	MEcopy(&raat_cb->recnum, sizeof(i4),
    ...
    *((i4 *)(buf_ptr + msg_size)) =
	raat_cb->table_handle->table_info.tbl_width;
    msg_size += sizeof(i4);
    MEcopy(raat_cb->record, raat_cb->table_handle->table_info.tbl_width,
	buf_ptr + msg_size);
    msg_size += raat_cb->table_handle->table_info.tbl_width;
