...
**
**  Description:
**	This file becomes the DLL for the Ingres VISION Merge Module.
**	It contains custom actions that are required to be executed.
**
**	    ingres_set_vision_reg_entries	- Set the appropriate registry
**						  entries for the post install.
**	    ingres_remove_vision_files		- Removes directories created
**						  during the VISION post
**						  install.
**
**  History:
**	18-may-2001 (somsa01)
**	    Created.
**	22-may-2001 (somsa01)
**	    In ingres_set_vision_reg_entries(), grab and set the service
...
**	18-jun-2003 (penga03)
**	    Do NOT set registry entries for VISION if either of following two cases 
**	    happens: 
**	    1) Feature IngresVision is not published
**	    2) IngresVision is not and will not be installed
**	26-jun-2003 (penga03)
**	    In ingres_rollback_vision() and ingres_unset_vision_reg_entries(), reset 
...

/*{
**  Name: ingres_set_vision_reg_entries
**
**  Description:
**	Sets the appropriate registry entries for the VISION Merge
**	Module.
**	These entries will drive the post installation for VISION.
**
**  Inputs:
...
**	    is needed.
**	05-jun-2002 (penga03)
**	    ONLY if no other products referencing VISION (by checking 
**	    the Bin component in VISION merge module), then add it to 
**	    RemovedFeatures.
**	13-Mar-2007 (drivi01)
...
UINT __stdcall
ingres_set_vision_reg_entries (MSIHANDLE ihnd)
{
    HKEY		hKey;
    char		KeyName[256];
    int			status = 0;
    ...
    if (!MsiGetFeatureState(ihnd, "IngresVision", &iInstalled, &iAction))
    {
	...
	*/
	...
	{
	    strcpy(tchValue, "VISION");
	}
	else if (!strstr(tchValue, "VISION"))
	    strcat(tchValue, ",VISION");
    ...
    if (!_stricmp(szID, "II"))
	strcpy(szComponentGUID, "{395BC478-2F00-11D5-BDFC-00B0D0AD4485}");
    else
    {
	int idx;
	...
	sprintf(szComponentGUID, "{395BC478-2F00-11D5-%04X-%012X}", idx, idx*idx);
    }
	...
	*/
    ...
    status = RegSetValueEx( hKey, "Ingres/Vision", 0, REG_SZ, 
...

/*{
**  Name: ingres_remove_vision_files
**
**  Description:
**	Removes any files/directories created by the VISION Merge
...
**	    Created.
**	03-Aug-2001 (penga03)
**	    Let the VISION Merge Module delete the files/directories created 
**	    by the post installation process.
*/

UINT __stdcall
ingres_remove_vision_files (MSIHANDLE ihnd)
{
    char		KeyName[256];
    HKEY		hKey;
    DWORD		status = 0;
    ...
    */
    /*
    ** sprintf(tchValue, "%s\\ingres\\abf", tchII_SYSTEM);
...
/*
**  Name:ingres_rollback_vision
**
**  Description:
**	Reverse the changes to the system made by ingres_set_vision_reg_entries and 
**	ingres_remove_vision_files. Executed only during an installation rollback.
**
**  Inputs:
...
*/
UINT __stdcall
ingres_rollback_vision(MSIHANDLE hInstall)
{
	char ii_code[3];
	DWORD ret;
	...
	size=sizeof(data);
	if(RegQueryValueEx(hkRegKey, "Ingres/Vision", NULL, &type, (BYTE *)data, &size)!=ERROR_SUCCESS)
	{
		RegCloseKey(hkRegKey);
		return ERROR_SUCCESS;
	}
	
	RegDeleteValue(hkRegKey, "Ingres/Vision");
...
/*
**  Name:ingres_unset_vision_reg_entries
**
**  Description:
**	Reverse the changes to the system made by ingres_set_vision_reg_entries 
...
*/
UINT __stdcall
ingres_unset_vision_reg_entries(MSIHANDLE hInstall)
{
	char ii_code[3];
	DWORD ret;
	char RegKey[256], data[256];
