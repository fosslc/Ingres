...
**
**  Description:
**	This file becomes the DLL for the Ingres ICE Merge Module. It
**	contains custom actions that are required to be executed.
**
**	    ingres_set_ice_reg_entries	- Set the appropriate registry
**					  entries for the post install.
**	    ingres_remove_ice_files	- Removes directories created during
**					  the ICE post install.
**
**  History:
**	17-may-2001 (somsa01)
**	    Created.
**	22-may-2001 (somsa01)
**	    In ingres_set_ice_reg_entries(), grab and set the service
**	    info in the registry if not already set. Also, in
**	    ingres_remove_ice_files(), must set the Host variable before
**	    using it.
**	24-jan-2002 (penga03)
...
**	16-jul-2002 (penga03)
**	    Added a new registry entry, "iihttpserver", to indicate 
**	    which HTTP Server the user chosen.
**	18-jun-2003 (penga03)
**	    Do NOT set registry entries for ICE if either of following two cases 
**	    happens: 
**	    1) Feature IngresIce is not published
**	    2) IngresIce is not and will not be installed
**	26-jun-2003 (penga03)
**	    In ingres_rollback_ice() and ingres_unset_ice_reg_entries(), reset 
...

/*{
**  Name: ingres_set_ice_reg_entries
**
**  Description:
**	Sets the appropriate registry entries for the ICE Merge Module.
**	These entries will drive the post installation for the ICE server.
**
**  Inputs:
...
**	    is needed.
**	05-jun-2002 (penga03)
**	    ONLY if no other products referencing ICE (by checking the 
**	    Bin component in ICE merge module), then add it to 
**	    RemovedFeatures.
**	13-Mar-2007 (drivi01)
...
UINT __stdcall
ingres_set_ice_reg_entries (MSIHANDLE ihnd)
{
    HKEY		hKey;
    char		KeyName[256];
    int			status = 0;
    ...
    if (!MsiGetFeatureState(ihnd, "IngresIce", &iInstalled, &iAction))
    {
	...
	*/
    ...
    if (!_stricmp(szID, "II"))
	strcpy(szComponentGUID, "{95BA2258-2F07-11D5-BDFC-00B0D0AD4485}");
    else
    {
	int idx;
	...
	sprintf(szComponentGUID, "{95BA2258-2F07-11D5-%04X-%012X}", idx, idx*idx);
    }
	...
	*/
    ...
    dwSize = sizeof(tchValue);
    if (status ||
	MsiGetProperty( ihnd, "INGRES_HTTP_SERVER", tchValue,
			&dwSize ) != ERROR_SUCCESS)
    {
	return (ERROR_INSTALL_FAILURE);
    }
	...
	status = RegSetValueEx( hKey, "iihttpserver", 0, REG_SZ, tchValue,
				strlen(tchValue) + 1 );
    }

    dwSize = sizeof(tchValue);
    if (status ||
	MsiGetProperty( ihnd, "INGRES_HTTP_SERVER_PATH", tchValue,
			&dwSize ) != ERROR_SUCCESS)
    {
	return (ERROR_INSTALL_FAILURE);
    }
	...
	if (tchValue[strlen(tchValue)-1] == '\\')
	    tchValue[strlen(tchValue)-1] = '\0';
	status = RegSetValueEx( hKey, "iihttpserverdir", 0, REG_SZ, tchValue,
				strlen(tchValue) + 1 );
    }

    dwSize = sizeof(tchValue);
    if (status ||
	MsiGetProperty( ihnd, "INGRES_COLDFUSION_PATH", tchValue,
			&dwSize ) != ERROR_SUCCESS)
    {
	return (ERROR_INSTALL_FAILURE);
    }
	...
	if (tchValue[strlen(tchValue)-1] == '\\')
	    tchValue[strlen(tchValue)-1] = '\0';
	status = RegSetValueEx( hKey, "iicoldfusiondir", 0, REG_SZ, tchValue,
				strlen(tchValue) + 1 );
    }

    strcpy(tchValue, "TRUE");
    status = RegSetValueEx( hKey, "Ingres/ICE", 0, REG_SZ, 
...

/*{
**  Name: ingres_remove_ice_files
**
**  Description:
**	Removes any files/directories created by the ICE Merge Module
...
**	21-mar-2002 (penga03)
**	    Donnot remove ice directory here, this will be handled 
**	    during setup.
*/

UINT __stdcall
ingres_remove_ice_files (MSIHANDLE ihnd)
{
    char		KeyName[256];
    char		cmd[2048];
    char		Host[33];
    ...
    */
    /*
    ** sprintf(tchValue, "%s\\ingres\\ice", tchII_SYSTEM);
    ...
    */
    Local_PMhost(Host);
    sprintf(cmd, "iisetres.exe ii.%s.ingstart.*.icesvr 0", Host);
    Execute(cmd, TRUE);
...
/*
**  Name:ingres_rollback_ice
**
**  Description:
**	Reverse the changes to the system made by ingres_set_ice_reg_entries and 
**	ingres_remove_ice_files. Executed only during an installation rollback.
**
**  Inputs:
...
*/
UINT __stdcall
ingres_rollback_ice(MSIHANDLE hInstall)
{
	...
	if(RegQueryValueEx(hkRegKey, "RemovedFeatures", NULL, &type, (BYTE *)data, &size)==ERROR_SUCCESS)
	{
		...
		if(bRemoved)
		{
			...
			Local_PMhost(Host);
			sprintf(buf, "iisetres.exe ii.%s.ingstart.*.icesvr 1", Host);
			Execute(buf, TRUE);
		}
		if(*data==0)
	...
	size=sizeof(data);
	if(RegQueryValueEx(hkRegKey, "Ingres/ICE", NULL, &type, (BYTE *)data, &size)!=ERROR_SUCCESS)
	{
		RegCloseKey(hkRegKey);
		return ERROR_SUCCESS;
	}
	
	RegDeleteValue(hkRegKey, "Ingres/ICE");
	RegDeleteValue(hkRegKey, "iihttpserver");
	RegDeleteValue(hkRegKey, "iihttpserverdir");
	RegDeleteValue(hkRegKey, "iicoldfusiondir");
...
/*
**  Name:ingres_unset_ice_reg_entries
**
**  Description:
**	Reverse the changes to the system made by ingres_set_ice_reg_entries 
...
*/
UINT __stdcall
ingres_unset_ice_reg_entries(MSIHANDLE hInstall)
{
	char ii_code[3];
	DWORD ret;
	char RegKey[256], data[256];
