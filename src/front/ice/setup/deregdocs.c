...
**
** Description:
**      Deregister a list of files from the ice repository either from a
...
**	09-May-2001 (hanje04)
**	    Added SETUPLIB to NEEDLIBS to build in UNIX
*/
/*
PROGRAM=        deregdocs
...
*/
ICE_STATUS
deregisterdoc( HICECTX icectx, PARAMS* prog, PREGPARAMS rp )
{
    ...
    */
    ...
    {
        if ((status = ic_deletedoc( icectx, file, (ext) ? ext : "", locid ))
            == OK)
        {
            SIprintf("%s: Unit:%s Location:%s %s.%s deregistered\n",
                prog->name, unitname, location, file, ext);
        }
        else
        {
            II_CHAR* errmsg;
...
**
** Description:
**      Function drives the deregistration loop according to whether the
    ...
    */
    if ((status = ic_initialize( prog->node, user, passwd, NULL, &icectx )) == OK)
    {
        ...
        if (prog->inpfile)
        {
            ...
            **
            ** -u unit -l location filename.ext
            */
            FILE* inpfile = NULL;
            ...
            if (inpfile != NULL)
            {
                ...
                {
                    status = deregisterdoc( icectx, prog, &regparams );
                    if(regparams.owner)
                    {
...
main( II_INT argc, II_CHAR** argv)
{
    ...
    if (argc == 1)
    {
        IIUGerr( E_WU001B_DEREGDOCS_USAGE, UG_ERR_ERROR, 0, NULL, NULL,
        ...
        if (opt)
        {
            if (*argv[i] == '-')
            {
                ...
                switch(*option)
                {
                    ...
                    default:
                        printf("Ignored unknown option -%c\n", *option);
                        break;
                }
            }
            else
