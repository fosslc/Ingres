...
**    created
** 09-Jul-2001 (hajne04)
**    Removed references to filebuf::sh_read for MAINWIN as filebuf does 
**    not have this format on UNIX
** 31-Oct-2001 (hanje04)
...
** 23-Jan-2002 (uk$so01)
**    (bug 106844). Additional fix: missing close file.
** 30-Jan-2002 (uk$so01)
...
extern "C" long GetRowCSV (long* rowLength, char* lpRow, long* returnByte)
{
	...
	BOOL bEnd = FALSE;
	if (dataPage1.GetKBToScan() == 0 || dataPage1.GetStopScanning() == FALSE)
	{
	...
	{
		FILE* pDataFile = pUserData4CopyHander->GetDataFile();
		ASSERT (pDataFile);

		if (!pDataFile)
			bEnd = TRUE;
		if (!bEnd && feof (pDataFile))
			bEnd = TRUE;
	}
	if (bEnd)
	{
	...
	if (dataPage1.GetKBToScan() == 0 || dataPage1.GetStopScanning() == FALSE) // The Total file has been scanned:
	{
		...
		if (pUserData4CopyHander->GetUseFileDirectly())
		{
			...
			char* lpszRecord = T2A((LPTSTR)(LPCTSTR)strRecord);
			strcpy (lpRow, lpszRecord);
			strcat (lpRow, "\n");
			*returnByte  = strlen(lpszRecord) +1;
		}
		else
		{
			CaRecord record;
	...
	else // The partial file has been scanned (or interrupted, in this case the records are not all in memory):
	{
		...
		{
			TCHAR* pLine = _fgetts (tchszText, 3501, pDataFile);
			if (!pLine)
				break;
			...
			if (nLen <= 3500)
			{
				...
				if (pUserData4CopyHander->GetUseFileDirectly())
				{
					char* lpszRecord = T2A((LPTSTR)(LPCTSTR)strText);
					strcpy (lpRow, lpszRecord);
					strcat (lpRow, "\n");
					*returnByte  = strlen(lpszRecord) +1;

					nCurrent+= strlen(lpszRecord) + 1;
				}
				else
				{
					//
	...
	if (hwndAnnimate && IsWindow(hwndAnnimate))
	{
...
extern "C" long GetRowDBF (long* rowLength, char* lpRow, long* returnByte)
{
	...
	// The records are already fetched into memory:
	if ((dataPage1.GetKBToScan() == 0 || dataPage1.GetStopScanning() == FALSE) && 
	    !pIIAData->GetInterrupted()) // The Total file has been scanned:
	{
		int& nCurrent = pUserData4CopyHander->GetCurrentRecord();
		//
		// Formating the record:
	...
	else // The partial file has been scanned (or interrupted, in this case the records are not all in memory):
	{
		...
		char* lpszRecord = T2A((LPTSTR)(LPCTSTR)strOutRecord);
		strcpy (lpRow, lpszRecord);
		strcat (lpRow, "\n");
		*returnByte  = strlen(lpszRecord) +1;
...
int CaExecParamSQLCopy::Run(HWND hWndTaskDlg)
{
	...
	try
	{
		...
		if (bInterrupted || (dataPage1.GetKBToScan() > 0 && dataPage1.GetStopScanning()))
		{
			if (sepType == FMT_FIELDSEPARATOR || sepType == FMT_FIXEDWIDTH)
			{
				FILE* pDataFile = _tfopen((LPCTSTR)dataPage1.GetFile2BeImported(), _T("r"));
				if ( pDataFile == NULL )
				{
					throw (int)100; // cannot open file
				}
				pUserData4CopyHander->SetDataFile (pDataFile);
			}
			else
			{
				FILE* pDataFile = _tfopen((LPCTSTR)dataPage1.GetFile2BeImported(), _T("rb"));
				if (!pDataFile)
					throw (int)100; // cannot open file
				pUserData4CopyHander->SetDataFile (pDataFile);
		...
		{
			SetEnvironmentVariable(_T("II_DATE_FORMAT"), m_strIIDateFormat);
		}
...
#if defined (_USE_SETLOCKMODE_TIMEOUT)
		//
		// Activate time out for this session: 5
		pCurrentSession = sessionUsage.GetCurrentSession();
		if (pCurrentSession)
		{
		...
		sessionUsage.Release (SESSION_COMMIT); // Release and Commit:
		...
		case IMPORT_DBF:
			{
				...
				pFile = _tfopen ((LPCTSTR)strFile, _T("rb"));
				DBF_QueryRecord(hWndTaskDlg, m_pIIAInfo, pFile, strRecord, FALSE); // bOutRecord = FALSE (strRecord is not valid)
			}
			break;
		case IMPORT_XML: 
			break;
...
{
	if (!m_strException.IsEmpty())
	{
		::MessageBox(hWndTaskDlg, m_strException, m_strMsgBoxTitle, MB_ICONEXCLAMATION | MB_OK);
	}
}
...
int CaExecParamQueryRecords::Run(HWND hWndTaskDlg)
{
	...
	for (i=0; i<nSize; i++)
	{
		...
		if (hWndTaskDlg && IsWindow (hWndTaskDlg))
		{
