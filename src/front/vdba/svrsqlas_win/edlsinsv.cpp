...
/*
**    Source   : edlsinsv.cpp, Implementation File
...
CuEditableListCtrlSqlWizardInsertValue::CuEditableListCtrlSqlWizardInsertValue()
{
...
BEGIN_MESSAGE_MAP(CuEditableListCtrlSqlWizardInsertValue, CuEditableListCtrl)
	//{{AFX_MSG_MAP(CuEditableListCtrlSqlWizardInsertValue)
	ON_WM_LBUTTONDBLCLK()
	ON_WM_RBUTTONDOWN()
	//}}AFX_MSG_MAP
...
void CuEditableListCtrlSqlWizardInsertValue::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CuEditableListCtrlSqlWizardInsertValue::OnRButtonDown(UINT nFlags, CPoint point) 
{
...
LONG CuEditableListCtrlSqlWizardInsertValue::OnEditDlgOK (UINT wParam, LONG lParam)
{
	int iItem, iSubItem;
...
LONG CuEditableListCtrlSqlWizardInsertValue::OnEditAssistant (UINT wParam, LONG lParam)
{
	...
	//
	// It is table or view because we are inserting data to table/view:
...
void CuEditableListCtrlSqlWizardInsertValue::EditValue (int iItem, int iSubItem, CRect rcCell)
{
	if (iSubItem < 1)
		return;
