...
//    Source   : rvkgview.c
//
//    Dialog box for revoking privilege from view.  (General)
//
********************************************************************/
...
BOOL CALLBACK __export DlgRvkgviewDlgProc
...
*/
{
   ...
   lpProc = MakeProcInstance ((FARPROC) DlgRvkgviewDlgProc, hInst);
   retVal = VdbaDialogBoxParam
       (hResource,
        MAKEINTRESOURCE (IDD_RVKGVIEW),
        hwndOwner,
        (DLGPROC) lpProc,
        (LPARAM)  lprevoke
       );
...
BOOL CALLBACK __export DlgRvkgviewDlgProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
   switch (message)
   {
...
static BOOL OnInitDialog (HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
   ...
   //
   CATOListDummy();
   LoadString (hResource, (UINT)IDS_T_RVKGVIEW,  szFormat, sizeof (szFormat));
   lpHelpStack = StackObject_PUSH (lpHelpStack, StackObject_INIT ((UINT)IDD_RVKGVIEW));
   ...
   Button_SetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_PRIV   ), TRUE);
   Button_SetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_CASCADE), TRUE);
   Button_SetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_USERS  ), TRUE);
...
static void OnCommand (HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
   HWND hwndGrantees       = GetDlgItem (hwnd, IDC_RVKGVIEW_GRANTEES);
   ...
   switch (id)
   {
       case IDOK:
       {
           ...
           lprevoke->grant_option = Button_GetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_GOPT));
           lprevoke->cascade      = Button_GetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_CASCADE));
           ...
           if (ires != RES_SUCCESS)
           {
               ...
               ErrorMessage ((UINT)IDS_E_REVOKE_VIEW_FAILED, ires);
               break;
           }
           else 
           {
       ...
       case IDC_RVKGVIEW_USERS:
           lprevoke->GranteeType = OT_USER;
           FillGrantedUsers (hwnd, lprevoke);
           EnableDisableOKButton  (hwnd);
           break;

       case IDC_RVKGVIEW_GROUPS:
           lprevoke->GranteeType = OT_GROUP;
           FillGrantedUsers (hwnd, lprevoke);
           EnableDisableOKButton  (hwnd);
           break;

       case IDC_RVKGVIEW_ROLES:
           lprevoke->GranteeType = OT_ROLE;
           FillGrantedUsers (hwnd, lprevoke);
           EnableDisableOKButton  (hwnd);
           break;

       case IDC_RVKGVIEW_PUBLIC:
           lprevoke->GranteeType = OT_PUBLIC;
           FillGrantedUsers (hwnd, lprevoke);
           EnableDisableOKButton  (hwnd);
           break;

       case IDC_RVKGVIEW_GRANTEES:
           EnableDisableOKButton  (hwnd);
           break;

       case IDC_RVKGVIEW_SELECT    :
       case IDC_RVKGVIEW_INSERT    :
       case IDC_RVKGVIEW_DELETE    :
       case IDC_RVKGVIEW_UPDATE    :
           FillGrantedUsers       (hwnd, lprevoke);
           EnableDisableOKButton  (hwnd);
           break;
   }
...
static void FillGrantedUsers (HWND hwnd, LPREVOKEPARAMS lprevoke)
{
   int  i, Who;
   int  first = -1;
   HWND hwndGrantee=GetDlgItem (hwnd, IDC_RVKGVIEW_GRANTEES);
   ...
   lprevoke->Privileges [GRANT_SELECT   ] = Button_GetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_SELECT  ));
   lprevoke->Privileges [GRANT_INSERT   ] = Button_GetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_INSERT  ));
   lprevoke->Privileges [GRANT_UPDATE   ] = Button_GetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_UPDATE  ));
   lprevoke->Privileges [GRANT_DELETE   ] = Button_GetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_DELETE  ));

   lch = AddItemToList    (hwndGrantee);
   CAListBox_ResetContent (hwndGrantee);

   if (Button_GetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_USERS)))
   {
       Who = OT_USER;
       //CAListBoxFillUsers (hwndGrantee);
   }
   else
   if (Button_GetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_GROUPS)))
   {
       Who = OT_GROUP;
       //CAListBoxFillGroups (hwndGrantee);
   }
   else
   if (Button_GetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_ROLES)))
   {
       Who = OT_ROLE;
       //CAListBoxFillRoles (hwndGrantee);
   }
   else
   if (Button_GetCheck (GetDlgItem (hwnd, IDC_RVKGVIEW_PUBLIC)))
       Who = OT_PUBLIC;
...
static LPCHECKEDOBJECTS GetGrantee (LPREVOKEPARAMS lprevoke, int objectToFind, int grantee)
{
   ...
   char    szWithOwner [MAXOBJECTNAME];
   LPUCHAR parentstrings [MAXPLEVEL];
   ...
   {
       case GRANT_SELECT   :
           grant_user = OT_VIEWGRANT_SEL_USER;
           break; 
       case GRANT_INSERT   :
           grant_user = OT_VIEWGRANT_INS_USER;
           break;
       case GRANT_UPDATE   :
           grant_user = OT_VIEWGRANT_UPD_USER;
           break;
       case GRANT_DELETE   :
           grant_user = OT_VIEWGRANT_DEL_USER;
           break;
       default:
           grant_user = -1;
           break;
   ...
   parentstrings [1] = StringWithOwner ((LPTSTR)Quote4DisplayIfNeeded(lprevoke->PreselectObject),
                                        lprevoke->PreselectObjectOwner,
                                        szWithOwner);                       // View name with owner

   ires = DOMGetFirstObject (hdl, grant_user, 2, parentstrings, bwsystem, NULL, buf, NULL, NULL);
