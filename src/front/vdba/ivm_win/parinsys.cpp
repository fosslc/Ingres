...
**
**    Source   : parinsys.cpp , Implementation File
...
**	27-Sep-2001 (noifr01)
**	    (sir 105798) increased the width of the "description"
**	    column (the new description being very long)
**	01-nov-2001 (somsa01)
...
CuDlgParameterInstallationSystem::CuDlgParameterInstallationSystem(CWnd* pParent /*=NULL*/)
	: CDialog(CuDlgParameterInstallationSystem::IDD, pParent)
{
	//{{AFX_DATA_INIT(CuDlgParameterInstallationSystem)
...
void CuDlgParameterInstallationSystem::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CuDlgParameterInstallationSystem)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CuDlgParameterInstallationSystem, CDialog)
	//{{AFX_MSG_MAP(CuDlgParameterInstallationSystem)
	ON_WM_DESTROY()
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgParameterInstallationSystem message handlers

void CuDlgParameterInstallationSystem::PostNcDestroy() 
{
...
void CuDlgParameterInstallationSystem::OnDestroy() 
{
	CDialog::OnDestroy();
...
BOOL CuDlgParameterInstallationSystem::OnInitDialog() 
{
...
void CuDlgParameterInstallationSystem::OnSize(UINT nType, int cx, int cy) 
{
...
void CuDlgParameterInstallationSystem::RefreshParameter(BOOL bShowUnset)
{
	try
	{
		...
		if (theApp.m_ingresVariable.QueryEnvironment (m_listParameter, bShowUnset))
		{
			//
			// Sort the parameter:
	...
	{
		AfxMessageBox (_T("System error (CuDlgParameterInstallationSystem::RefreshParameter): \nFail to refresh parameter"));
	}
}

void CuDlgParameterInstallationSystem::OnItemchangedList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (pNMListView->iItem >= 0 && pNMListView->uNewState > 0 && (pNMListView->uNewState&LVIS_SELECTED))
	{
		...
		if (pParameter && !pParameter->IsReadOnly())
		{
			nState |= ST_MODIFY;
...
LONG CuDlgParameterInstallationSystem::OnEnableControl(WPARAM w, LPARAM l)
{
	UINT nState = ST_ADD;
	int iNameCol  = 0;
	int iValueCol = 1;
...
LONG CuDlgParameterInstallationSystem::OnListCtrlEditing (WPARAM w, LPARAM l)
{
	CRect r, rCell;
	int iValueCol = 1;
	int nSelected = m_cListCtrl.GetNextItem (-1, LVNI_SELECTED);
...
void CuDlgParameterInstallationSystem::SetUnsetVariable (BOOL bUnset)
{
...
LONG CuDlgParameterInstallationSystem::OnListCtrlEditOK(WPARAM w, LPARAM l)
{
	SetUnsetVariable (FALSE); // SET
	return 0;
}

LONG CuDlgParameterInstallationSystem::OnUnsetVariable(WPARAM w, LPARAM l)
{
...
void CuDlgParameterInstallationSystem::OnColumnclickList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_LISTVIEW* pNMListView = (NM_LISTVIEW*)pNMHDR;
...
LONG CuDlgParameterInstallationSystem::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LONG CuDlgParameterInstallationSystem::OnAddParameter (WPARAM wParam, LPARAM lParam)
{
	CaModalUp modalUp;
...
LONG CuDlgParameterInstallationSystem::OnFindAccepted (WPARAM wParam, LPARAM lParam)
{
	return (LONG)TRUE;
}


LONG CuDlgParameterInstallationSystem::OnFind (WPARAM wParam, LPARAM lParam)
{
	TRACE0 ("CuDlgParameterInstallationSystem::OnFind \n");
