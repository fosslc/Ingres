// dgdpdvls.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CuDlgDomPropViewLinkSource, CFormView)

CuDlgDomPropViewLinkSource::CuDlgDomPropViewLinkSource()
: CFormView(CuDlgDomPropViewLinkSource::IDD)
{
    //{{AFX_DATA_INIT(CuDlgDomPropViewLinkSource)
...
void CuDlgDomPropViewLinkSource::DoDataExchange(CDataExchange* pDX)
{
  CFormView::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropViewLinkSource)
...
BEGIN_MESSAGE_MAP(CuDlgDomPropViewLinkSource, CFormView)
    //{{AFX_MSG_MAP(CuDlgDomPropViewLinkSource)
    ON_WM_SIZE()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropViewLinkSource diagnostics

#ifdef _DEBUG
void CuDlgDomPropViewLinkSource::AssertValid() const
{
    CFormView::AssertValid();
}

void CuDlgDomPropViewLinkSource::Dump(CDumpContext& dc) const
{
    CFormView::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropViewLinkSource message handlers

void CuDlgDomPropViewLinkSource::OnSize(UINT nType, int cx, int cy) 
{
    CFormView::OnSize(nType, cx, cy);
}


LONG CuDlgDomPropViewLinkSource::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
...
LONG CuDlgDomPropViewLinkSource::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  if (iResult != RES_SUCCESS) {
    ...
    // Reset m_Data
    CuDomPropDataViewLinkSource tempData;
...
LONG CuDlgDomPropViewLinkSource::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataViewLinkSource") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataViewLinkSource* pData = (CuDomPropDataViewLinkSource*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataViewLinkSource)));
...
LONG CuDlgDomPropViewLinkSource::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataViewLinkSource* pData = new CuDomPropDataViewLinkSource(m_Data);
...
void CuDlgDomPropViewLinkSource::RefreshDisplay()
{
...
void CuDlgDomPropViewLinkSource::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
