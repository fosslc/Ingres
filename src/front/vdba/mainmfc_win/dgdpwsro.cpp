// DgDpwsro.cpp : implementation file
//
...
CuDlgDomPropIceSecRoles::CuDlgDomPropIceSecRoles(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropIceSecRoles::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropIceSecRoles)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropIceSecRoles::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropIceSecRoles)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropIceSecRoles::AddRole (CuNameOnly *pIceSecRoles)
{
  ASSERT (pIceSecRoles);
  BOOL  bSpecial = pIceSecRoles->IsSpecialItem();

  int nCount, index;
  nCount = m_cListCtrl.GetItemCount ();

  // Name - last parameter is Image Index
  index  = m_cListCtrl.InsertItem (nCount, (LPCTSTR)pIceSecRoles->GetStrName(), 0);

  // Attach pointer on object, for future sort purposes
  m_cListCtrl.SetItemData(index, (DWORD)pIceSecRoles);
}


BEGIN_MESSAGE_MAP(CuDlgDomPropIceSecRoles, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropIceSecRoles)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropIceSecRoles message handlers

void CuDlgDomPropIceSecRoles::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropIceSecRoles::OnInitDialog() 
{
...
void CuDlgDomPropIceSecRoles::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropIceSecRoles::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropIceSecRoles::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  switch (pUps->nIpmHint)
  {
    ...
    case FILTER_DOM_BKREFRESH:
      ...
      if (pUps->pSFilter->UpdateType != OT_VIRTNODE &&
          pUps->pSFilter->UpdateType != OT_ICE_ROLE)
        return 0L;
      break;
  ...
  // Get list of Roles
  //
  m_Data.m_uaIceSecRoles.RemoveAll();
  ...
  ASSERT (lpRecord->recType == OT_STATIC_ICE_SECURITY
       || lpRecord->recType == OT_STATIC_ICE_ROLE);
...
LONG CuDlgDomPropIceSecRoles::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataIceSecRoles") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataIceSecRoles* pData = (CuDomPropDataIceSecRoles*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataIceSecRoles)));
...
LONG CuDlgDomPropIceSecRoles::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataIceSecRoles* pData = new CuDomPropDataIceSecRoles(m_Data);
...
void CuDlgDomPropIceSecRoles::RefreshDisplay()
{
  ...
  size = m_Data.m_uaIceSecRoles.GetCount();
  for (cnt = 0; cnt < size; cnt++)
    AddRole(m_Data.m_uaIceSecRoles[cnt]);
}

void CuDlgDomPropIceSecRoles::OnColumnclickMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropIceSecRoles::OnDblclkMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	*pResult = 0;
    ...
    if (pFastObject) {
      if (!pFastObject->IsSpecialItem()) {
        ...
        case OT_STATIC_ICE_SECURITY:
            domTreeFastItemList.AddTail(new CuDomTreeFastItem(OT_STATIC_ICE_ROLE));
            break;
        ...
        domTreeFastItemList.AddTail(new CuDomTreeFastItem(OT_ICE_ROLE,
...
void CuDlgDomPropIceSecRoles::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
