// DgDpDdTb.cpp : implementation file
//
...
CuDlgDomPropDDbTbl::CuDlgDomPropDDbTbl(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropDDbTbl::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropDDbTbl)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropDDbTbl::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropDDbTbl)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropDDbTbl::AddTbl (CuNameWithOwner *pDbTbl)
{
  ASSERT (pDbTbl);
  ...
  int imageIndex = 0;
  if (!bSpecial) {
    BOOL bNative = pDbTbl->GetFlag(FLAG_STARTYPE);
    if (bNative)
      imageIndex = 2;
    else
      imageIndex = 1;
  ...
  index  = m_cListCtrl.InsertItem (nCount, (LPCTSTR)pDbTbl->GetStrName(), imageIndex);
...
BEGIN_MESSAGE_MAP(CuDlgDomPropDDbTbl, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropDDbTbl)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropDDbTbl message handlers

void CuDlgDomPropDDbTbl::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropDDbTbl::OnInitDialog() 
{
  ...
  m_StarImageList.Create(IDB_IMGLIST_STARTBL, 16, 1, RGB(255, 0, 255));
...
void CuDlgDomPropDDbTbl::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropDDbTbl::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropDDbTbl::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
    ...
    while (iret == RES_SUCCESS) {
      int iType = getint(bufComplim + STEPSMALLOBJ);
...
LONG CuDlgDomPropDDbTbl::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LONG CuDlgDomPropDDbTbl::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropDDbTbl::RefreshDisplay()
{
...
void CuDlgDomPropDDbTbl::OnColumnclickMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropDDbTbl::OnDblclkMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
  *pResult = 0;
...
void CuDlgDomPropDDbTbl::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
