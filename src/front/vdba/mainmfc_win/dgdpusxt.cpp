...
CuDlgDomPropUserXGrantedTbl::CuDlgDomPropUserXGrantedTbl(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropUserXGrantedTbl::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropUserXGrantedTbl)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropUserXGrantedTbl::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropUserXGrantedTbl)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropUserXGrantedTbl::AddUserXGrantedTbl (CuGrantedTbl* pGrantedTbl)
{
  ASSERT (pGrantedTbl);
  ...
  m_cListCtrl.SetItemData(index, (DWORD)pGrantedTbl);
}


BEGIN_MESSAGE_MAP(CuDlgDomPropUserXGrantedTbl, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropUserXGrantedTbl)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropUserXGrantedTbl message handlers

void CuDlgDomPropUserXGrantedTbl::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropUserXGrantedTbl::OnInitDialog() 
{
...
void CuDlgDomPropUserXGrantedTbl::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropUserXGrantedTbl::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropUserXGrantedTbl::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  //
  m_Data.m_uaUserXGrantedTbl.RemoveAll();
    ...
    iret =  DOMGetFirstRelObject(nNodeHandle,
     ...
     if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
      ...
      while (iret == RES_SUCCESS) {
        ...
        CuMultFlag *pRefGrantedTbl = m_Data.m_uaUserXGrantedTbl.Find(&grantedTbl);
        if (pRefGrantedTbl)
          m_Data.m_uaUserXGrantedTbl.Merge(pRefGrantedTbl, &grantedTbl);
        else
          m_Data.m_uaUserXGrantedTbl.Add(grantedTbl);
  ...
  if (bError)
  {
    ...
    m_Data.m_uaUserXGrantedTbl.Add(grantedTbl1);
  }

  // Manage no granted object
  if (m_Data.m_uaUserXGrantedTbl.GetCount() == 0)
  {
    /* "<No GrantedTbl>" */
    CuGrantedTbl grantedTbl2(VDBA_MfcResourceString (IDS_E_NO_GRANTEDTBL));
    m_Data.m_uaUserXGrantedTbl.Add(grantedTbl2);
  }

  ASSERT (m_Data.m_uaUserXGrantedTbl.GetCount() > 0 );
...
LONG CuDlgDomPropUserXGrantedTbl::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataUserXGrantedTbl") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataUserXGrantedTbl* pData = (CuDomPropDataUserXGrantedTbl*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataUserXGrantedTbl)));
...
LONG CuDlgDomPropUserXGrantedTbl::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataUserXGrantedTbl* pData = new CuDomPropDataUserXGrantedTbl(m_Data);
...
void CuDlgDomPropUserXGrantedTbl::RefreshDisplay()
{
  ...
  size = m_Data.m_uaUserXGrantedTbl.GetCount();
  ASSERT (size > 0);
  for (cnt = 0; cnt < size; cnt++) {
    CuGrantedTbl* pGrantedTbl = m_Data.m_uaUserXGrantedTbl[cnt];
    AddUserXGrantedTbl(pGrantedTbl);
  }
  // No Initial sort
}

void CuDlgDomPropUserXGrantedTbl::OnColumnClickList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropUserXGrantedTbl::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
