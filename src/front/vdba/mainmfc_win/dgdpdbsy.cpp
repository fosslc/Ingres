// DgDpDbSy.cpp : implementation file
//
...
CuDlgDomPropDbSyn::CuDlgDomPropDbSyn(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropDbSyn::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropDbSyn)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropDbSyn::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropDbSyn)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropDbSyn::AddSyn (CuNameWithOwner *pDbSyn)
{
  ASSERT (pDbSyn);
  BOOL  bSpecial = pDbSyn->IsSpecialItem();

  int nCount, index;
  nCount = m_cListCtrl.GetItemCount ();

  // Name - last parameter is Image Index
  index  = m_cListCtrl.InsertItem (nCount, (LPCTSTR)pDbSyn->GetStrName(), 0);

  // Owner
  if (!bSpecial)
    m_cListCtrl.SetItemText (index, 1, pDbSyn->GetOwnerName());

  // Attach pointer on object, for future sort purposes
  m_cListCtrl.SetItemData(index, (DWORD)pDbSyn);
}


BEGIN_MESSAGE_MAP(CuDlgDomPropDbSyn, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropDbSyn)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropDbSyn message handlers

void CuDlgDomPropDbSyn::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropDbSyn::OnInitDialog() 
{
  ...
  m_ImageList.AddIcon(IDI_TREE_STATIC_SYNONYM);
...
void CuDlgDomPropDbSyn::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropDbSyn::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropDbSyn::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  switch (pUps->nIpmHint)
  {
    ...
    case FILTER_DOM_BKREFRESH:
      ...
      if (pUps->pSFilter->UpdateType != OT_VIRTNODE &&
          pUps->pSFilter->UpdateType != OT_SYNONYM)
        return 0L;
      break;
  ...
  //
  // Get list of Syn for the replication
  //
  m_Data.m_uaDbSyn.RemoveAll();
  ...
  if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
    ...
    m_Data.m_uaDbSyn.Add(errItem);
  }
  else {
    while (iret == RES_SUCCESS) {
      CuNameWithOwner item((const char*)buf, (const char*)bufOwner);
      m_Data.m_uaDbSyn.Add(item);

      iret = DOMGetNextObject(buf, bufOwner, bufComplim);
    }
  }
  if (m_Data.m_uaDbSyn.GetCount() == 0) {
    CuNameWithOwner noItem(VDBA_MfcResourceString (IDS_E_NO_SYNONYM));//"<No Synonym>"
    m_Data.m_uaDbSyn.Add(noItem);
  }
...
LONG CuDlgDomPropDbSyn::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataDbSyn") == 0);
  ResetDisplay();

  // Get class pointer, and check for it's class runtime type
  CuDomPropDataDbSyn* pData = (CuDomPropDataDbSyn*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataDbSyn)));
...
LONG CuDlgDomPropDbSyn::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataDbSyn* pData = new CuDomPropDataDbSyn(m_Data);
...
void CuDlgDomPropDbSyn::RefreshDisplay()
{
  ...
  size = m_Data.m_uaDbSyn.GetCount();
  for (cnt = 0; cnt < size; cnt++)
    AddSyn(m_Data.m_uaDbSyn[cnt]);
}

void CuDlgDomPropDbSyn::OnColumnclickMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropDbSyn::OnDblclkMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
  *pResult = 0;
    ...
    if (pFastObject) {
      if (!pFastObject->IsSpecialItem()) {
        ...
        switch (m_Data.m_objType) {
          case OT_DATABASE:
            domTreeFastItemList.AddTail(new CuDomTreeFastItem(OT_STATIC_SYNONYM));
            break;
          case OT_STATIC_SYNONYM:
            break;
          default:
        ...
        domTreeFastItemList.AddTail(new CuDomTreeFastItem(OT_SYNONYM,
...
void CuDlgDomPropDbSyn::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
