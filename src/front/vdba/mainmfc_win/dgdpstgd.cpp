// dgdpstgd.cpp : implementation file
//
...
CuDlgDomPropStaticGwDb::CuDlgDomPropStaticGwDb(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropStaticGwDb::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropStaticGwDb)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropStaticGwDb::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropStaticGwDb)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropStaticGwDb::AddGwDb (CuNameOnly *pGwDb)
{
  ASSERT (pGwDb);
  BOOL  bSpecial = pGwDb->IsSpecialItem();

  int nCount, index;
  nCount = m_cListCtrl.GetItemCount ();

  // Name - last parameter is Image Index
  index  = m_cListCtrl.InsertItem (nCount, (LPCTSTR)pGwDb->GetStrName(), 0);

  // Attach pointer on object, for future sort purposes
  m_cListCtrl.SetItemData(index, (DWORD)pGwDb);
}


BEGIN_MESSAGE_MAP(CuDlgDomPropStaticGwDb, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropStaticGwDb)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropStaticGwDb message handlers

void CuDlgDomPropStaticGwDb::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropStaticGwDb::OnInitDialog() 
{
...
void CuDlgDomPropStaticGwDb::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropStaticGwDb::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropStaticGwDb::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  // Get list of GwDbs
  //
  m_Data.m_uaStaticGwDb.RemoveAll();
  ...
  if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
    ...
    m_Data.m_uaStaticGwDb.Add(errItem);
  }
  else {
    while (iret == RES_SUCCESS) {
      CuNameOnly item((const char*)buf, FALSE);   // Not special, but regular
      m_Data.m_uaStaticGwDb.Add(item);

      iret = DOMGetNextObject(buf, bufOwner, bufComplim);
    }
  }
  if (m_Data.m_uaStaticGwDb.GetCount() == 0) {
    CuNameOnly noItem(VDBA_MfcResourceString (IDS_E_NO_GWDB), TRUE);//"<No GwDb>"
    m_Data.m_uaStaticGwDb.Add(noItem);
  }
...
LONG CuDlgDomPropStaticGwDb::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataStaticGwDb") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataStaticGwDb* pData = (CuDomPropDataStaticGwDb*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataStaticGwDb)));
...
LONG CuDlgDomPropStaticGwDb::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataStaticGwDb* pData = new CuDomPropDataStaticGwDb(m_Data);
...
void CuDlgDomPropStaticGwDb::RefreshDisplay()
{
  ...
  size = m_Data.m_uaStaticGwDb.GetCount();
  ASSERT (size > 0);

  for (cnt = 0; cnt < size; cnt++)
    AddGwDb(m_Data.m_uaStaticGwDb[cnt]);
}

void CuDlgDomPropStaticGwDb::OnColumnclickMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropStaticGwDb::OnDblclkMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	*pResult = 0;
...
void CuDlgDomPropStaticGwDb::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
