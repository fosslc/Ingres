...
// DgDpwfpu.cpp : implementation file
//
// 13-Dec-2001 (noifr01)
...
CuDlgDomPropIceFacetNPageUsers::CuDlgDomPropIceFacetNPageUsers(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropIceFacetNPageUsers::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropIceFacetNPageUsers)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropIceFacetNPageUsers::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropIceFacetNPageUsers)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropIceFacetNPageUsers::AddFacetNPageAccessDef (CuIceFacetNPageAccessdef *pIceFacetNPageAccessDef)
{
...
BEGIN_MESSAGE_MAP(CuDlgDomPropIceFacetNPageUsers, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropIceFacetNPageUsers)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropIceFacetNPageUsers message handlers

void CuDlgDomPropIceFacetNPageUsers::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropIceFacetNPageUsers::OnInitDialog() 
{
...
void CuDlgDomPropIceFacetNPageUsers::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropIceFacetNPageUsers::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropIceFacetNPageUsers::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  ASSERT (lpRecord->recType == OT_STATIC_ICE_BUNIT_FACET_USER ||
          lpRecord->recType == OT_ICE_BUNIT_FACET             ||
          lpRecord->recType == OT_STATIC_ICE_BUNIT_PAGE_USER ||
  ...
  switch (lpRecord->recType) {
    case OT_ICE_BUNIT_FACET:
    case OT_STATIC_ICE_BUNIT_FACET_USER:
      reqType = OT_ICE_BUNIT_FACET_USER;
      break;
    case OT_ICE_BUNIT_PAGE:
    case OT_STATIC_ICE_BUNIT_PAGE_USER:
      reqType = OT_ICE_BUNIT_PAGE_USER;
      break;
    default:
      ASSERT(FALSE);
      return 0L;
  ...
  if (m_Data.m_uaIceFacetNPageAccessdef.GetCount() == 0) {
    CuIceFacetNPageAccessdef noItem(VDBA_MfcResourceString(IDS_NO_USER));//"< No User >"      // Special item
...
LONG CuDlgDomPropIceFacetNPageUsers::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LONG CuDlgDomPropIceFacetNPageUsers::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropIceFacetNPageUsers::RefreshDisplay()
{
...
void CuDlgDomPropIceFacetNPageUsers::OnColumnclickMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropIceFacetNPageUsers::OnDblclkMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	*pResult = 0;
    ...
    if (pFastObject) {
      if (!pFastObject->IsSpecialItem()) {
        ...
        switch (m_Data.m_objType) {
          case OT_ICE_BUNIT_FACET:
            domTreeFastItemList.AddTail(new CuDomTreeFastItem(OT_STATIC_ICE_BUNIT_FACET_USER));
            // NOTE: No break to chain on reqType
          case OT_STATIC_ICE_BUNIT_FACET_USER:
            finalType = OT_ICE_BUNIT_FACET_USER;
            break;

          case OT_ICE_BUNIT_PAGE:
            domTreeFastItemList.AddTail(new CuDomTreeFastItem(OT_STATIC_ICE_BUNIT_PAGE_USER));
            // NOTE: No break to chain on reqType
          case OT_STATIC_ICE_BUNIT_PAGE_USER:
            finalType = OT_ICE_BUNIT_PAGE_USER;
            break;
...
void CuDlgDomPropIceFacetNPageUsers::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
