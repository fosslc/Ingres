// DgDpwp.cpp : implementation file
//
...
CuDlgDomPropIcePages::CuDlgDomPropIcePages(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropIcePages::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropIcePages)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropIcePages::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropIcePages)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropIcePages::AddPage (CuNameOnly *pIcePages)
{
  ASSERT (pIcePages);
  BOOL  bSpecial = pIcePages->IsSpecialItem();

  int nCount, index;
  nCount = m_cListCtrl.GetItemCount ();

  // Name - last parameter is Image Index
  index  = m_cListCtrl.InsertItem (nCount, (LPCTSTR)pIcePages->GetStrName(), 0);

  // Attach pointer on object, for future sort purposes
  m_cListCtrl.SetItemData(index, (DWORD)pIcePages);
}


BEGIN_MESSAGE_MAP(CuDlgDomPropIcePages, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropIcePages)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropIcePages message handlers

void CuDlgDomPropIcePages::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropIcePages::OnInitDialog() 
{
...
void CuDlgDomPropIcePages::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropIcePages::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropIcePages::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  switch (pUps->nIpmHint)
  {
    ...
    case FILTER_DOM_BKREFRESH:
      ...
      if (pUps->pSFilter->UpdateType != OT_VIRTNODE &&
          pUps->pSFilter->UpdateType != OT_ICE_BUNIT_PAGE)
        return 0L;
      break;
  ...
  ASSERT (lpRecord->recType == OT_STATIC_ICE_BUNIT_PAGE || lpRecord->recType == OT_ICE_BUNIT);
  ...
  if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
    ...
    m_Data.m_uaIcePages.Add(errItem);
  }
  else {
    while (iret == RES_SUCCESS) {
      CuNameOnly item((const char*)buf, FALSE);   // Regular item
      m_Data.m_uaIcePages.Add(item);

      iret = DOMGetNextObject(buf, bufOwner, bufComplim);
    }
  }
  if (m_Data.m_uaIcePages.GetCount() == 0) {
    CuNameOnly noItem(VDBA_MfcResourceString(IDS_TREE_ICE_BUNIT_PAGE_NONE), TRUE);//"< No Page >"      // Special item
    m_Data.m_uaIcePages.Add(noItem);
  }
...
LONG CuDlgDomPropIcePages::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataIcePages") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataIcePages* pData = (CuDomPropDataIcePages*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataIcePages)));
...
LONG CuDlgDomPropIcePages::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataIcePages* pData = new CuDomPropDataIcePages(m_Data);
...
void CuDlgDomPropIcePages::RefreshDisplay()
{
  ...
  size = m_Data.m_uaIcePages.GetCount();
  for (cnt = 0; cnt < size; cnt++)
    AddPage(m_Data.m_uaIcePages[cnt]);
}

void CuDlgDomPropIcePages::OnColumnclickMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropIcePages::OnDblclkMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	*pResult = 0;
  ...
  if (selItemId != -1) {
    ...
    if (pFastObject) {
      if (!pFastObject->IsSpecialItem()) {
        ...
        case OT_ICE_BUNIT:
            domTreeFastItemList.AddTail(new CuDomTreeFastItem(OT_STATIC_ICE_BUNIT_PAGE));
            break;
          case OT_STATIC_ICE_BUNIT_PAGE:
            break;
          default:
        ...
        domTreeFastItemList.AddTail(new CuDomTreeFastItem(OT_ICE_BUNIT_PAGE,
...
void CuDlgDomPropIcePages::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
