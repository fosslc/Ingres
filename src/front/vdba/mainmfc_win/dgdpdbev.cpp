...
void CuDlgDomPropDbEvt::AddEvt (CuNameWithOwner *pDbEvt)
{
  ...
  m_cListCtrl.SetItemData(index, (DWORD)pDbEvt);
}
...
LONG CuDlgDomPropDbEvt::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropDbEvt::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LONG CuDlgDomPropDbEvt::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LONG CuDlgDomPropDbEvt::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropDbEvt::RefreshDisplay()
{
  ...
  size = m_Data.m_uaDbEvt.GetCount();
