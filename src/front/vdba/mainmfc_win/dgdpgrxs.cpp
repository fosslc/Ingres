...
/*
**
**  Name: dgdpgrxs.cpp: implementation file
**
**  Description:
**
**  History:
...
void CuDlgDomPropGroupXGrantedSeq::AddGroupXGrantedSeq (CuGrantedSeq* pGrantedSeq)
{
  ASSERT (pGrantedSeq);
...
CuDlgDomPropGroupXGrantedSeq::CuDlgDomPropGroupXGrantedSeq(CWnd* pParent /*=NULL*/)
	: CDialog(CuDlgDomPropGroupXGrantedSeq::IDD, pParent)
{
	//{{AFX_DATA_INIT(CuDlgDomPropGroupXGrantedSeq)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}


void CuDlgDomPropGroupXGrantedSeq::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CuDlgDomPropGroupXGrantedSeq)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CuDlgDomPropGroupXGrantedSeq, CDialog)
	//{{AFX_MSG_MAP(CuDlgDomPropGroupXGrantedSeq)
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropGroupXGrantedSeq message handlers

void CuDlgDomPropGroupXGrantedSeq::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropGroupXGrantedSeq::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropGroupXGrantedSeq::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  //
  m_Data.m_uaGroupXGrantedSeq.RemoveAll();
    ...
    iret =  DOMGetFirstRelObject(nNodeHandle,
     ...
     if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
      ...
      while (iret == RES_SUCCESS) {
        ...
        CuMultFlag *pRefGrantedSeq = m_Data.m_uaGroupXGrantedSeq.Find(&grantedSeq);
        if (pRefGrantedSeq)
          m_Data.m_uaGroupXGrantedSeq.Merge(pRefGrantedSeq, &grantedSeq);
        else
          m_Data.m_uaGroupXGrantedSeq.Add(grantedSeq);
  ...
  if (bError)
  {
    ...
    m_Data.m_uaGroupXGrantedSeq.Add(grantedSeq1);
  }

  // Manage no granted object
  if (m_Data.m_uaGroupXGrantedSeq.GetCount() == 0)
  {
    /* "<No GrantedSeq>" */
    CuGrantedSeq grantedSeq2(VDBA_MfcResourceString (IDS_E_NO_GRANTED_SEQ));
    m_Data.m_uaGroupXGrantedSeq.Add(grantedSeq2);
  }

  ASSERT (m_Data.m_uaGroupXGrantedSeq.GetCount() > 0 );
...
LONG CuDlgDomPropGroupXGrantedSeq::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataGroupXGrantedSeq") == 0);
  ResetDisplay();

  // Get class pointer, and check for it's class runtime type
  CuDomPropDataGroupXGrantedSeq* pData = (CuDomPropDataGroupXGrantedSeq*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataGroupXGrantedSeq)));
...
LONG CuDlgDomPropGroupXGrantedSeq::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataGroupXGrantedSeq* pData = new CuDomPropDataGroupXGrantedSeq(m_Data);
...
void CuDlgDomPropGroupXGrantedSeq::RefreshDisplay()
{
  ...
  size = m_Data.m_uaGroupXGrantedSeq.GetCount();
  ASSERT (size > 0);
  for (cnt = 0; cnt < size; cnt++) {
    CuGrantedSeq* pGrantedSeq = m_Data.m_uaGroupXGrantedSeq[cnt];
    AddGroupXGrantedSeq(pGrantedSeq);
  }
  // No Initial sort
}


void CuDlgDomPropGroupXGrantedSeq::OnColumnclickMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropGroupXGrantedSeq::PostNcDestroy() 
{
	delete this;
	CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropGroupXGrantedSeq::OnInitDialog() 
{
...
void CuDlgDomPropGroupXGrantedSeq::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
