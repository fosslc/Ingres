...
**	    and IDS_E_NO_GRRANTED_DB into two statements to avoid compiler
**	    error.
**  09-Apr-2003 (schph01)
...
CuDlgDomPropRoleXGrantedDb::CuDlgDomPropRoleXGrantedDb(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropRoleXGrantedDb::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropRoleXGrantedDb)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropRoleXGrantedDb::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropRoleXGrantedDb)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropRoleXGrantedDb::AddRoleXGrantedDb (CuGrantedDb* pGrantedDb)
{
  ASSERT (pGrantedDb);
...
BEGIN_MESSAGE_MAP(CuDlgDomPropRoleXGrantedDb, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropRoleXGrantedDb)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropRoleXGrantedDb message handlers

void CuDlgDomPropRoleXGrantedDb::PostNcDestroy() 
{
  delete this;
...
void CuDlgDomPropRoleXGrantedDb::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropRoleXGrantedDb::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropRoleXGrantedDb::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataRoleXGrantedDb") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataRoleXGrantedDb* pData = (CuDomPropDataRoleXGrantedDb*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataRoleXGrantedDb)));
...
LONG CuDlgDomPropRoleXGrantedDb::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataRoleXGrantedDb* pData = new CuDomPropDataRoleXGrantedDb(m_Data);
...
void CuDlgDomPropRoleXGrantedDb::RefreshDisplay()
{
  ...
  size = m_Data.m_uaRoleXGrantedDb.GetCount();
  ASSERT (size > 0);
  for (cnt = 0; cnt < size; cnt++) {
    CuGrantedDb* pGrantedDb = m_Data.m_uaRoleXGrantedDb[cnt];
    AddRoleXGrantedDb(pGrantedDb);
  }
  // No Initial sort
}

void CuDlgDomPropRoleXGrantedDb::OnColumnClickList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropRoleXGrantedDb::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
