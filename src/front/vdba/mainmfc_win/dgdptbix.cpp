// DgDpTbIx.cpp : implementation file
//
...
CuDlgDomPropTblIdx::CuDlgDomPropTblIdx(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropTblIdx::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropTblIdx)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropTblIdx::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropTblIdx)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropTblIdx::AddTblIdx (CuNameWithOwner *pTblIdx)
{
  ASSERT (pTblIdx);
...
BEGIN_MESSAGE_MAP(CuDlgDomPropTblIdx, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropTblIdx)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropTblIdx message handlers

void CuDlgDomPropTblIdx::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropTblIdx::OnInitDialog() 
{
...
void CuDlgDomPropTblIdx::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropTblIdx::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropTblIdx::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  switch (pUps->nIpmHint)
  {
    ...
    case FILTER_DOM_BKREFRESH:
      ...
      if (pUps->pSFilter->UpdateType != OT_VIRTNODE &&
          pUps->pSFilter->UpdateType != OT_INDEX)
        return 0L;
      break;
...
LONG CuDlgDomPropTblIdx::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LONG CuDlgDomPropTblIdx::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropTblIdx::RefreshDisplay()
{
...
void CuDlgDomPropTblIdx::OnColumnclickMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropTblIdx::OnDblclkMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	*pResult = 0;
...
void CuDlgDomPropTblIdx::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
