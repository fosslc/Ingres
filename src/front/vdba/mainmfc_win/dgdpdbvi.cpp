...
CuDlgDomPropDbView::CuDlgDomPropDbView(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropDbView::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropDbView)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropDbView::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropDbView)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropDbView::AddView (CuNameWithOwner *pDbView)
{
  ASSERT (pDbView);
  ...
  index  = m_cListCtrl.InsertItem (nCount, (LPCTSTR)pDbView->GetStrName(), 0);
...
BEGIN_MESSAGE_MAP(CuDlgDomPropDbView, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropDbView)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropDbView message handlers

void CuDlgDomPropDbView::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropDbView::OnInitDialog() 
{
...
void CuDlgDomPropDbView::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropDbView::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropDbView::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LONG CuDlgDomPropDbView::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LONG CuDlgDomPropDbView::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropDbView::RefreshDisplay()
{
...
void CuDlgDomPropDbView::OnColumnclickMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropDbView::OnDblclkMfcList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
  *pResult = 0;
...
void CuDlgDomPropDbView::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
