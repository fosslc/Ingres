...
**	    and IDS_E_GRANTED_VIEW into two statements to avoid compiler
**	    error.
*/
...
CuDlgDomPropGroupXGrantedView::CuDlgDomPropGroupXGrantedView(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropGroupXGrantedView::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropGroupXGrantedView)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropGroupXGrantedView::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropGroupXGrantedView)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropGroupXGrantedView::AddGroupXGrantedView (CuGrantedView* pGrantedView)
{
  ASSERT (pGrantedView);
...
BEGIN_MESSAGE_MAP(CuDlgDomPropGroupXGrantedView, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropGroupXGrantedView)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropGroupXGrantedView message handlers

void CuDlgDomPropGroupXGrantedView::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropGroupXGrantedView::OnInitDialog() 
{
...
void CuDlgDomPropGroupXGrantedView::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropGroupXGrantedView::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropGroupXGrantedView::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  //
  m_Data.m_uaGroupXGrantedView.RemoveAll();
    ...
    iret =  DOMGetFirstRelObject(nNodeHandle,
     ...
     if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
      ...
      while (iret == RES_SUCCESS) {
        ...
        CuMultFlag *pRefGrantedView = m_Data.m_uaGroupXGrantedView.Find(&grantedView);
        if (pRefGrantedView)
          m_Data.m_uaGroupXGrantedView.Merge(pRefGrantedView, &grantedView);
        else
          m_Data.m_uaGroupXGrantedView.Add(grantedView);
  ...
  if (bError)
  {
    ...
    m_Data.m_uaGroupXGrantedView.Add(grantedView1);
  }

  // Manage no granted object
  if (m_Data.m_uaGroupXGrantedView.GetCount() == 0)
  {
    /* "<No GrantedView>" */
    CuGrantedView grantedView2(VDBA_MfcResourceString (IDS_E_GRANTED_VIEW));
    m_Data.m_uaGroupXGrantedView.Add(grantedView2);
  }

  ASSERT (m_Data.m_uaGroupXGrantedView.GetCount() > 0 );
...
LONG CuDlgDomPropGroupXGrantedView::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataGroupXGrantedView") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataGroupXGrantedView* pData = (CuDomPropDataGroupXGrantedView*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataGroupXGrantedView)));
...
LONG CuDlgDomPropGroupXGrantedView::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataGroupXGrantedView* pData = new CuDomPropDataGroupXGrantedView(m_Data);
...
void CuDlgDomPropGroupXGrantedView::RefreshDisplay()
{
  ...
  size = m_Data.m_uaGroupXGrantedView.GetCount();
  ASSERT (size > 0);
  for (cnt = 0; cnt < size; cnt++) {
    CuGrantedView* pGrantedView = m_Data.m_uaGroupXGrantedView[cnt];
    AddGroupXGrantedView(pGrantedView);
  }
  // No Initial sort
}

void CuDlgDomPropGroupXGrantedView::OnColumnClickList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropGroupXGrantedView::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
