...
CuDlgDomPropUserXGrantedView::CuDlgDomPropUserXGrantedView(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropUserXGrantedView::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropUserXGrantedView)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropUserXGrantedView::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropUserXGrantedView)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropUserXGrantedView::AddUserXGrantedView (CuGrantedView* pGrantedView)
{
  ASSERT (pGrantedView);
...
BEGIN_MESSAGE_MAP(CuDlgDomPropUserXGrantedView, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropUserXGrantedView)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropUserXGrantedView message handlers

void CuDlgDomPropUserXGrantedView::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropUserXGrantedView::OnInitDialog() 
{
...
void CuDlgDomPropUserXGrantedView::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropUserXGrantedView::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropUserXGrantedView::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  //
  m_Data.m_uaUserXGrantedView.RemoveAll();
    ...
    iret =  DOMGetFirstRelObject(nNodeHandle,
     ...
     if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
      ...
      while (iret == RES_SUCCESS) {
        ...
        CuMultFlag *pRefGrantedView = m_Data.m_uaUserXGrantedView.Find(&grantedView);
        if (pRefGrantedView)
          m_Data.m_uaUserXGrantedView.Merge(pRefGrantedView, &grantedView);
        else
          m_Data.m_uaUserXGrantedView.Add(grantedView);
  ...
  if (bError)
  {
    ...
    m_Data.m_uaUserXGrantedView.Add(grantedView1);
  }

  // Manage no granted object
  if (m_Data.m_uaUserXGrantedView.GetCount() == 0)
  {
    /* "<No GrantedView>" */
    CuGrantedView grantedView2(VDBA_MfcResourceString (IDS_E_GRANTED_VIEW));
    m_Data.m_uaUserXGrantedView.Add(grantedView2);
  }

  ASSERT (m_Data.m_uaUserXGrantedView.GetCount() > 0 );
...
LONG CuDlgDomPropUserXGrantedView::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataUserXGrantedView") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataUserXGrantedView* pData = (CuDomPropDataUserXGrantedView*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataUserXGrantedView)));
...
LONG CuDlgDomPropUserXGrantedView::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataUserXGrantedView* pData = new CuDomPropDataUserXGrantedView(m_Data);
...
void CuDlgDomPropUserXGrantedView::RefreshDisplay()
{
  ...
  size = m_Data.m_uaUserXGrantedView.GetCount();
  ASSERT (size > 0);
  for (cnt = 0; cnt < size; cnt++) {
    CuGrantedView* pGrantedView = m_Data.m_uaUserXGrantedView[cnt];
    AddUserXGrantedView(pGrantedView);
  }
  // No Initial sort
}

void CuDlgDomPropUserXGrantedView::OnColumnClickList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropUserXGrantedView::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
