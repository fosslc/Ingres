...
**	    and IDS_E_NO_GRANTED_DB into two statements to avoid compiler
**	    error.
**  09-Apr-2003 (schph01)
...
CuDlgDomPropUserXGrantedDb::CuDlgDomPropUserXGrantedDb(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropUserXGrantedDb::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropUserXGrantedDb)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropUserXGrantedDb::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropUserXGrantedDb)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropUserXGrantedDb::AddUserXGrantedDb (CuGrantedDb* pGrantedDb)
{
  ASSERT (pGrantedDb);
...
BEGIN_MESSAGE_MAP(CuDlgDomPropUserXGrantedDb, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropUserXGrantedDb)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropUserXGrantedDb message handlers

void CuDlgDomPropUserXGrantedDb::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropUserXGrantedDb::OnInitDialog() 
{
...
void CuDlgDomPropUserXGrantedDb::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropUserXGrantedDb::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropUserXGrantedDb::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  //
  m_Data.m_uaUserXGrantedDb.RemoveAll();
    ...
    iret =  DOMGetFirstRelObject(nNodeHandle,
     ...
     if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
      ...
      while (iret == RES_SUCCESS) {
        ...
        CuMultFlag *pRefGrantedDb = m_Data.m_uaUserXGrantedDb.Find(&grantedDb);
        if (pRefGrantedDb)
          m_Data.m_uaUserXGrantedDb.Merge(pRefGrantedDb, &grantedDb);
        else
          m_Data.m_uaUserXGrantedDb.Add(grantedDb);
  ...
  if (bError)
  {
    ...
    m_Data.m_uaUserXGrantedDb.Add(grantedDb1);
  }

  // Manage no granted object
  if (m_Data.m_uaUserXGrantedDb.GetCount() == 0)
  {
    /* "<No GrantedDb>" */
    CuGrantedDb grantedDb2(VDBA_MfcResourceString (IDS_E_NO_GRANTED_DB));
    m_Data.m_uaUserXGrantedDb.Add(grantedDb2);
  }

  ASSERT (m_Data.m_uaUserXGrantedDb.GetCount() > 0 );
...
LONG CuDlgDomPropUserXGrantedDb::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataUserXGrantedDb") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataUserXGrantedDb* pData = (CuDomPropDataUserXGrantedDb*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataUserXGrantedDb)));
...
LONG CuDlgDomPropUserXGrantedDb::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataUserXGrantedDb* pData = new CuDomPropDataUserXGrantedDb(m_Data);
...
void CuDlgDomPropUserXGrantedDb::RefreshDisplay()
{
  ...
  size = m_Data.m_uaUserXGrantedDb.GetCount();
  ASSERT (size > 0);
  for (cnt = 0; cnt < size; cnt++) {
    CuGrantedDb* pGrantedDb = m_Data.m_uaUserXGrantedDb[cnt];
    AddUserXGrantedDb(pGrantedDb);
  }
  // No Initial sort
}

void CuDlgDomPropUserXGrantedDb::OnColumnClickList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropUserXGrantedDb::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
