...
CuDlgDomPropUserXGrantedEvent::CuDlgDomPropUserXGrantedEvent(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropUserXGrantedEvent::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropUserXGrantedEvent)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropUserXGrantedEvent::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropUserXGrantedEvent)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropUserXGrantedEvent::AddUserXGrantedEvent (CuGrantedEvent* pGrantedEvent)
{
...
BEGIN_MESSAGE_MAP(CuDlgDomPropUserXGrantedEvent, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropUserXGrantedEvent)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropUserXGrantedEvent message handlers

void CuDlgDomPropUserXGrantedEvent::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropUserXGrantedEvent::OnInitDialog() 
{
...
void CuDlgDomPropUserXGrantedEvent::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropUserXGrantedEvent::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropUserXGrantedEvent::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  //
  m_Data.m_uaUserXGrantedEvent.RemoveAll();
    ...
    iret =  DOMGetFirstRelObject(nNodeHandle,
     ...
     if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
      ...
      while (iret == RES_SUCCESS) {
        ...
        CuMultFlag *pRefGrantedEvent = m_Data.m_uaUserXGrantedEvent.Find(&grantedEvent);
        if (pRefGrantedEvent)
          m_Data.m_uaUserXGrantedEvent.Merge(pRefGrantedEvent, &grantedEvent);
        else
          m_Data.m_uaUserXGrantedEvent.Add(grantedEvent);
  ...
  if (bError)
  {
    ...
    m_Data.m_uaUserXGrantedEvent.Add(grantedEvent1);
  }

  // Manage no granted object
  if (m_Data.m_uaUserXGrantedEvent.GetCount() == 0)
  {
    /* "<No GrantedEvent>" */
    CuGrantedEvent grantedEvent2(VDBA_MfcResourceString (IDS_E_NO_GRANT_EVENT));
    m_Data.m_uaUserXGrantedEvent.Add(grantedEvent2);
  }

  ASSERT (m_Data.m_uaUserXGrantedEvent.GetCount() > 0 );
...
LONG CuDlgDomPropUserXGrantedEvent::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataUserXGrantedEvent") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataUserXGrantedEvent* pData = (CuDomPropDataUserXGrantedEvent*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataUserXGrantedEvent)));
...
LONG CuDlgDomPropUserXGrantedEvent::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataUserXGrantedEvent* pData = new CuDomPropDataUserXGrantedEvent(m_Data);
...
void CuDlgDomPropUserXGrantedEvent::RefreshDisplay()
{
  ...
  size = m_Data.m_uaUserXGrantedEvent.GetCount();
  ASSERT (size > 0);
  for (cnt = 0; cnt < size; cnt++) {
    CuGrantedEvent* pGrantedEvent = m_Data.m_uaUserXGrantedEvent[cnt];
    AddUserXGrantedEvent(pGrantedEvent);
  }
  // No Initial sort
}

void CuDlgDomPropUserXGrantedEvent::OnColumnClickList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropUserXGrantedEvent::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
