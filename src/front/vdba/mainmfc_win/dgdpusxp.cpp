...
CuDlgDomPropUserXGrantedProc::CuDlgDomPropUserXGrantedProc(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropUserXGrantedProc::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropUserXGrantedProc)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropUserXGrantedProc::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropUserXGrantedProc)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropUserXGrantedProc::AddUserXGrantedProc (CuGrantedProc* pGrantedProc)
{
  ASSERT (pGrantedProc);
...
BEGIN_MESSAGE_MAP(CuDlgDomPropUserXGrantedProc, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropUserXGrantedProc)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropUserXGrantedProc message handlers

void CuDlgDomPropUserXGrantedProc::PostNcDestroy() 
{
  delete this;    
  CDialog::PostNcDestroy();
}

BOOL CuDlgDomPropUserXGrantedProc::OnInitDialog() 
{
...
void CuDlgDomPropUserXGrantedProc::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropUserXGrantedProc::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropUserXGrantedProc::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  //
  m_Data.m_uaUserXGrantedProc.RemoveAll();
    ...
    iret =  DOMGetFirstRelObject(nNodeHandle,
     ...
     if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
      ...
      while (iret == RES_SUCCESS) {
        ...
        CuMultFlag *pRefGrantedProc = m_Data.m_uaUserXGrantedProc.Find(&grantedProc);
        if (pRefGrantedProc)
          m_Data.m_uaUserXGrantedProc.Merge(pRefGrantedProc, &grantedProc);
        else
          m_Data.m_uaUserXGrantedProc.Add(grantedProc);
  ...
  if (bError)
  {
    ...
    m_Data.m_uaUserXGrantedProc.Add(grantedProc1);
  }

  // Manage no granted object
  if (m_Data.m_uaUserXGrantedProc.GetCount() == 0)
  {
    /* "<No GrantedProc>" */
    CuGrantedProc grantedProc2(VDBA_MfcResourceString (IDS_E_NO_GRANTED_PROC));
    m_Data.m_uaUserXGrantedProc.Add(grantedProc2);
  }

  ASSERT (m_Data.m_uaUserXGrantedProc.GetCount() > 0 );
...
LONG CuDlgDomPropUserXGrantedProc::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataUserXGrantedProc") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataUserXGrantedProc* pData = (CuDomPropDataUserXGrantedProc*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataUserXGrantedProc)));
...
LONG CuDlgDomPropUserXGrantedProc::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataUserXGrantedProc* pData = new CuDomPropDataUserXGrantedProc(m_Data);
...
void CuDlgDomPropUserXGrantedProc::RefreshDisplay()
{
  ...
  size = m_Data.m_uaUserXGrantedProc.GetCount();
  ASSERT (size > 0);
  for (cnt = 0; cnt < size; cnt++) {
    CuGrantedProc* pGrantedProc = m_Data.m_uaUserXGrantedProc[cnt];
    AddUserXGrantedProc(pGrantedProc);
  }
  // No Initial sort
}

void CuDlgDomPropUserXGrantedProc::OnColumnClickList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropUserXGrantedProc::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
