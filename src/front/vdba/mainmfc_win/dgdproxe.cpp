...
CuDlgDomPropRoleXGrantedEvent::CuDlgDomPropRoleXGrantedEvent(CWnd* pParent /*=NULL*/)
    : CDialog(CuDlgDomPropRoleXGrantedEvent::IDD, pParent)
{
    //{{AFX_DATA_INIT(CuDlgDomPropRoleXGrantedEvent)
        // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
}


void CuDlgDomPropRoleXGrantedEvent::DoDataExchange(CDataExchange* pDX)
{
  CDialog::DoDataExchange(pDX);
  //{{AFX_DATA_MAP(CuDlgDomPropRoleXGrantedEvent)
      // NOTE: the ClassWizard will add DDX and DDV calls here
  //}}AFX_DATA_MAP
}

void CuDlgDomPropRoleXGrantedEvent::AddRoleXGrantedEvent (CuGrantedEvent* pGrantedEvent)
{
...
BEGIN_MESSAGE_MAP(CuDlgDomPropRoleXGrantedEvent, CDialog)
    //{{AFX_MSG_MAP(CuDlgDomPropRoleXGrantedEvent)
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDomPropRoleXGrantedEvent message handlers

void CuDlgDomPropRoleXGrantedEvent::PostNcDestroy() 
{
  delete this;    
...
void CuDlgDomPropRoleXGrantedEvent::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgDomPropRoleXGrantedEvent::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LONG CuDlgDomPropRoleXGrantedEvent::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  //
  m_Data.m_uaRoleXGrantedEvent.RemoveAll();
    ...
    iret =  DOMGetFirstRelObject(nNodeHandle,
     ...
     if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
      ...
      while (iret == RES_SUCCESS) {
        ...
        CuMultFlag *pRefGrantedEvent = m_Data.m_uaRoleXGrantedEvent.Find(&grantedEvent);
        if (pRefGrantedEvent)
          m_Data.m_uaRoleXGrantedEvent.Merge(pRefGrantedEvent, &grantedEvent);
        else
          m_Data.m_uaRoleXGrantedEvent.Add(grantedEvent);
  ...
  if (bError)
  {
    ...
    m_Data.m_uaRoleXGrantedEvent.Add(grantedEvent1);
  }

  // Manage no granted object
  if (m_Data.m_uaRoleXGrantedEvent.GetCount() == 0)
  {
    /* "<No GrantedEvent>" */
    CuGrantedEvent grantedEvent2(VDBA_MfcResourceString (IDS_E_NO_GRANT_EVENT));
    m_Data.m_uaRoleXGrantedEvent.Add(grantedEvent2);
  }

  ASSERT (m_Data.m_uaRoleXGrantedEvent.GetCount() > 0 );
...
LONG CuDlgDomPropRoleXGrantedEvent::OnLoad (WPARAM wParam, LPARAM lParam)
{
  // Precheck consistency of class name
  LPCTSTR pClass = (LPCTSTR)wParam;
  ASSERT (lstrcmp (pClass, "CuDomPropDataRoleXGrantedEvent") == 0);
  ResetDisplay();
  // Get class pointer, and check for it's class runtime type
  CuDomPropDataRoleXGrantedEvent* pData = (CuDomPropDataRoleXGrantedEvent*)lParam;
  ASSERT (pData->IsKindOf(RUNTIME_CLASS(CuDomPropDataRoleXGrantedEvent)));
...
LONG CuDlgDomPropRoleXGrantedEvent::OnGetData (WPARAM wParam, LPARAM lParam)
{
  // Allocate data class instance WITH NEW, and
  // DUPLICATE current m_Data in the allocated instance using copy constructor
  CuDomPropDataRoleXGrantedEvent* pData = new CuDomPropDataRoleXGrantedEvent(m_Data);
...
void CuDlgDomPropRoleXGrantedEvent::RefreshDisplay()
{
  ...
  size = m_Data.m_uaRoleXGrantedEvent.GetCount();
  ASSERT (size > 0);
  for (cnt = 0; cnt < size; cnt++) {
    CuGrantedEvent* pGrantedEvent = m_Data.m_uaRoleXGrantedEvent[cnt];
    AddRoleXGrantedEvent(pGrantedEvent);
  }
  // No Initial sort
}

void CuDlgDomPropRoleXGrantedEvent::OnColumnClickList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuDlgDomPropRoleXGrantedEvent::ResetDisplay()
{
  UpdateData (FALSE);   // Mandatory!
  m_cListCtrl.DeleteAllItems();
  VDBA_OnGeneralSettingChange(&m_cListCtrl);
