...
**
**    Source   : cachedrv.cpp, Implementation File 
...
CuDlgDbmsCacheDerived::CuDlgDbmsCacheDerived(CWnd* pParent /*=NULL*/)
	: CDialog(CuDlgDbmsCacheDerived::IDD, pParent)
{
	//{{AFX_DATA_INIT(CuDlgDbmsCacheDerived)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}


void CuDlgDbmsCacheDerived::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CuDlgDbmsCacheDerived)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CuDlgDbmsCacheDerived, CDialog)
	//{{AFX_MSG_MAP(CuDlgDbmsCacheDerived)
	ON_WM_DESTROY()
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgDbmsCacheDerived message handlers

void CuDlgDbmsCacheDerived::OnDestroy() 
{
	VCBF_GenericDerivedDestroyItemData (&m_ListCtrl);
	CDialog::OnDestroy();
}

BOOL CuDlgDbmsCacheDerived::OnInitDialog() 
{
...
void CuDlgDbmsCacheDerived::OnSize(UINT nType, int cx, int cy) 
{
...
void CuDlgDbmsCacheDerived::PostNcDestroy() 
{
	delete this;
...
LONG CuDlgDbmsCacheDerived::OnValidateData (WPARAM wParam, LPARAM lParam)
{
	if (!IsWindow (m_ListCtrl.m_hWnd))
		return 0;
...
LONG CuDlgDbmsCacheDerived::OnButton1 (UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgDbmsCacheDerived::OnButton1.(Edit Value)..\n");
	CRect r, rCell;
	UINT nState = 0;
	int iNameCol = 1;
	int iIndex = -1;
...
LONG CuDlgDbmsCacheDerived::OnButton2(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgDbmsCacheDerived::OnButton2.(Recalculate)..\n");
	...
	if (iIndex == -1)
		return 0;
	m_ListCtrl.HideProperty();
	VCBF_GenericDerivedRecalculate (m_ListCtrl, iIndex, TRUE);
	return 0;
}

LONG CuDlgDbmsCacheDerived::OnButton3(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgDbmsCacheDerived::OnButton3...\n");
	return 0;
}

LONG CuDlgDbmsCacheDerived::OnButton4(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgDbmsCacheDerived::OnButton4...\n");
	return 0;
}

LONG CuDlgDbmsCacheDerived::OnButton5(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgDbmsCacheDerived::OnButton5...\n");
	return 0;
}

void CuDlgDbmsCacheDerived::OnItemchangedList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
