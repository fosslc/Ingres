...
**
**    Source   : msqpadlg.cpp, Implementation File 
...
CuDlgGW_MSSQLParameter::CuDlgGW_MSSQLParameter(CWnd* pParent /*=NULL*/)
	: CDialog(CuDlgGW_MSSQLParameter::IDD, pParent)
{
	//{{AFX_DATA_INIT(CuDlgGW_MSSQLParameter)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}


void CuDlgGW_MSSQLParameter::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CuDlgGW_MSSQLParameter)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CuDlgGW_MSSQLParameter, CDialog)
	//{{AFX_MSG_MAP(CuDlgGW_MSSQLParameter)
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgGW_MSSQLParameter message handlers

void CuDlgGW_MSSQLParameter::PostNcDestroy() 
{
	delete this;
	CDialog::PostNcDestroy();
}

void CuDlgGW_MSSQLParameter::OnDestroy() 
{
	VCBF_GenericDestroyItemData (&m_ListCtrl);
	CDialog::OnDestroy();
}

BOOL CuDlgGW_MSSQLParameter::OnInitDialog() 
{
...
void CuDlgGW_MSSQLParameter::OnSize(UINT nType, int cx, int cy) 
{
...
void CuDlgGW_MSSQLParameter::OnItemchangedList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
LONG CuDlgGW_MSSQLParameter::OnButton1 (UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgGW_MSSQLParameter::OnButton1 (Edit Value)...\n");
	CRect r, rCell;
	UINT nState = 0;
	int iNameCol = 1;
	int iIndex = -1;
...
LONG CuDlgGW_MSSQLParameter::OnButton2(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgGW_MSSQLParameter::OnButton2...\n");
	UINT nState = 0;
	int iIndex = -1;
	int i, nCount = m_ListCtrl.GetItemCount();
	for (i=0; i<nCount; i++)
...
LONG CuDlgGW_MSSQLParameter::OnButton3(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgGW_MSSQLParameter::OnButton3...\n");
	return 0;
}

LONG CuDlgGW_MSSQLParameter::OnButton4(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgGW_MSSQLParameter::OnButton4...\n");
	return 0;
}

LONG CuDlgGW_MSSQLParameter::OnButton5(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgGW_MSSQLParameter::OnButton5...\n");
	return 0;
}


LONG CuDlgGW_MSSQLParameter::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
	CWnd* pParent1 = GetParent ();              // CTabCtrl
	ASSERT_VALID (pParent1);
	CWnd* pParent2 = pParent1->GetParent ();    // CConfRightDlg
...
LONG CuDlgGW_MSSQLParameter::OnValidateData (WPARAM wParam, LPARAM lParam)
{
	m_ListCtrl.HideProperty();
	return 0L;
}