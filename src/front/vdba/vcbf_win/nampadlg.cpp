...
** 
**    Source   : nampadlg.cpp, Implementation File 
...
CuDlgNameParameter::CuDlgNameParameter(CWnd* pParent)
	: CDialog(CuDlgNameParameter::IDD, pParent)
{
	//{{AFX_DATA_INIT(CuDlgNameParameter)
	//}}AFX_DATA_INIT
}


void CuDlgNameParameter::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CuDlgNameParameter)
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CuDlgNameParameter, CDialog)
	//{{AFX_MSG_MAP(CuDlgNameParameter)
	ON_WM_DESTROY()
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgNameParameter message handlers


void CuDlgNameParameter::OnDestroy() 
{
	VCBF_GenericDestroyItemData (&m_ListCtrl);
	CDialog::OnDestroy();
}

BOOL CuDlgNameParameter::OnInitDialog() 
{
	VCBF_CreateControlGeneric (m_ListCtrl, this, &m_ImageList, &m_ImageListCheck);
	return TRUE;
}

LONG CuDlgNameParameter::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LONG CuDlgNameParameter::OnValidateData (WPARAM wParam, LPARAM lParam)
{
	m_ListCtrl.HideProperty();
	return 0L;
}

void CuDlgNameParameter::PostNcDestroy() 
{
	delete this;
...
void CuDlgNameParameter::OnSize(UINT nType, int cx, int cy) 
{
...
LONG CuDlgNameParameter::OnButton1 (UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgNameParameter::OnButton1 (Edit Value)...\n");
	CRect r, rCell;
	UINT nState = 0;
	int iNameCol = 1;
	int iIndex = -1;
...
LONG CuDlgNameParameter::OnButton2(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgNameParameter::OnButton2...\n");
	UINT nState = 0;
	int iIndex = -1;
	int i, nCount = m_ListCtrl.GetItemCount();
	for (i=0; i<nCount; i++)
...
LONG CuDlgNameParameter::OnButton3(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgNameParameter::OnButton3...\n");
	return 0;
}

LONG CuDlgNameParameter::OnButton4(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgNameParameter::OnButton4...\n");
	return 0;
}

LONG CuDlgNameParameter::OnButton5(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgNameParameter::OnButton5...\n");
	return 0;
}


void CuDlgNameParameter::OnItemchangedList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
