...
**
**    Source   : sybpadlg.cpp, Implementation File
**    Project  : OpenIngres Configuration Manager
**    Author   : Joseph Cuccia
**    Purpose  : Modeless Dialog, Page (Parameter) of Sybase Gateway 
**
** History:
**
** xx-Dec-1998 (cucjo01)
...
CuDlgGW_SybaseParameter::CuDlgGW_SybaseParameter(CWnd* pParent /*=NULL*/)
	: CDialog(CuDlgGW_SybaseParameter::IDD, pParent)
{
	//{{AFX_DATA_INIT(CuDlgGW_SybaseParameter)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}


void CuDlgGW_SybaseParameter::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CuDlgGW_SybaseParameter)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CuDlgGW_SybaseParameter, CDialog)
	//{{AFX_MSG_MAP(CuDlgGW_SybaseParameter)
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CuDlgGW_SybaseParameter message handlers

void CuDlgGW_SybaseParameter::PostNcDestroy() 
{
	delete this;
	CDialog::PostNcDestroy();
}

void CuDlgGW_SybaseParameter::OnDestroy() 
{
	VCBF_GenericDestroyItemData (&m_ListCtrl);
	CDialog::OnDestroy();
}

BOOL CuDlgGW_SybaseParameter::OnInitDialog() 
{
...
void CuDlgGW_SybaseParameter::OnSize(UINT nType, int cx, int cy) 
{
...
void CuDlgGW_SybaseParameter::OnItemchangedList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
LONG CuDlgGW_SybaseParameter::OnButton1 (UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgGW_SybaseParameter::OnButton1 (Edit Value)...\n");
	CRect r, rCell;
	UINT nState = 0;
	int iNameCol = 1;
	int iIndex = -1;
...
LONG CuDlgGW_SybaseParameter::OnButton2(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgGW_SybaseParameter::OnButton2...\n");
	UINT nState = 0;
	int iIndex = -1;
	int i, nCount = m_ListCtrl.GetItemCount();
	for (i=0; i<nCount; i++)
...
LONG CuDlgGW_SybaseParameter::OnButton3(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgGW_SybaseParameter::OnButton3...\n");
	return 0;
}

LONG CuDlgGW_SybaseParameter::OnButton4(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgGW_SybaseParameter::OnButton4...\n");
	return 0;
}

LONG CuDlgGW_SybaseParameter::OnButton5(UINT wParam, LONG lParam)
{
	TRACE0 ("CuDlgGW_SybaseParameter::OnButton5...\n");
	return 0;
}


LONG CuDlgGW_SybaseParameter::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
	CWnd* pParent1 = GetParent ();              // CTabCtrl
	ASSERT_VALID (pParent1);
	CWnd* pParent2 = pParent1->GetParent ();    // CConfRightDlg
...
LONG CuDlgGW_SybaseParameter::OnValidateData (WPARAM wParam, LPARAM lParam)
{
	m_ListCtrl.HideProperty();
	return 0L;
}