...
/*
**    Source   : comrule.cpp: implementation of the CmtRule, CmtFolderRule class
...
//                                 LOCAL FUNCTIONS
// ************************************************************************************************
static HRESULT CopyRules2Stream(CTypedPtrList< CObList, CmtRule* >& listObject, IStream*& pStream)
{
	...
	try
	{
		...
		CTypedPtrList <CObList, CaRule*> listObject2Store;

		CaRule* pObj = NULL;
		POSITION pos = listObject.GetHeadPosition();
		while (pos != NULL)
		{
			CmtRule* pMtObj = listObject.GetNext(pos);
			pMtObj->GetRule (pObj);
...
// Object: Rule
// *****************
CmtSessionManager* CmtRule::GetSessionManager()
{
...
CmtSessionManager* CmtFolderRule::GetSessionManager()
{
	return m_pBackParent? m_pBackParent->GetSessionManager(): NULL;
}

CmtRule* CmtFolderRule::SearchObject(CaLLQueryInfo* pInfo, CaRule* pObj, BOOL b2QueryIfNotDone)
{
	...
	while (pos != NULL)
	{
		CmtRule* pMtObj = m_listObject.GetNext (pos);
		CaRule& obj = pMtObj->GetRule();
		if (obj.Matched(pObj))
		{
			pMtObj->Access();
			return pMtObj;
...
HRESULT CmtFolderRule::GetListObject(CaLLQueryInfo* pInfo, IStream*& pStream, BOOL bCopy2Stream)
{
	HRESULT hError = NOERROR;
	pStream = NULL;
	//
	...
	if (GetLastQuery() != -1)
	{
		...
		mReadWrite.UnLock();
		if (bCopy2Stream)
			hError = CopyRules2Stream (m_listObject, pStream);
		return hError;
	}
	...
	while (!lNew.IsEmpty())
	{
		CaRule* pObj = (CaRule*)lNew.RemoveHead();
		CmtRule* pObjExist = SearchObject(pInfo, pObj, FALSE);
		...
		CmtRule* pNewMtObject = new CmtRule (pObj);
