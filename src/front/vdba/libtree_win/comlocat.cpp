...
/*
**    Source   : comlocat.cpp: implementation of the CmtLocation, CmtFolderLocation class
...
//                                 LOCAL FUNCTIONS
// ************************************************************************************************
static HRESULT CopyLocations2Stream(CTypedPtrList< CObList, CmtLocation* >& listObject, IStream*& pStream)
{
	...
	try
	{
		...
		CTypedPtrList <CObList, CaLocation*> listObject2Store;
		//
		// Write the individual object:
		CaLocation* pObj = NULL;
		POSITION pos = listObject.GetHeadPosition();
		while (pos != NULL)
		{
			CmtLocation* pMtObj = listObject.GetNext(pos);
			pMtObj->GetLocation (pObj);
...
// Object: Location
// ****************
CmtSessionManager* CmtLocation::GetSessionManager()
{
...
// Folder of Location:
// *******************
CmtFolderLocation::~CmtFolderLocation()
{
...
CmtSessionManager* CmtFolderLocation::GetSessionManager()
{
	return m_pBackParent? m_pBackParent->GetSessionManager(): NULL;
}

CmtLocation* CmtFolderLocation::SearchObject(CaLLQueryInfo* pInfo, CaLocation* pObj, BOOL b2QueryIfNotDone)
{
	...
	while (pos != NULL)
	{
		CmtLocation* pMtObj = m_listObject.GetNext (pos);
		CaLocation& obj = pMtObj->GetLocation();
		if (obj.Matched(pObj))
		{
			pMtObj->Access();
			return pMtObj;
...
HRESULT CmtFolderLocation::GetListObject(CaLLQueryInfo* pInfo, IStream*& pStream, BOOL bCopy2Stream)
{
	HRESULT hError = NOERROR;
	pStream = NULL;
	//
	...
	if (GetLastQuery() != -1)
	{
		...
		mReadWrite.UnLock();
		if (bCopy2Stream)
			hError = CopyLocations2Stream (m_listObject, pStream);
		return hError;
	}
	...
	pInfo->SetObjectType(OBT_LOCATION);
	...
	while (!lNew.IsEmpty())
	{
		CaLocation* pObj = (CaLocation*)lNew.RemoveHead();
		CmtLocation* pObjExist = SearchObject(pInfo, pObj, FALSE);
		...
		CmtLocation* pNewMtObject = new CmtLocation (pObj);
