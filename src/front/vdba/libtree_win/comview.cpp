...
/*
**    Source   : comview.cpp: implementation of the CmtView, CmtFolderView class
...
//                                 LOCAL FUNCTIONS
// ************************************************************************************************
static HRESULT CopyViews2Stream(CTypedPtrList< CObList, CmtView* >& listObject, IStream*& pStream)
{
	...
	try
	{
		...
		CTypedPtrList <CObList, CaView*> listObject2Store;
		//
		...
		while (pos != NULL)
		{
			CmtView* pMtObj = listObject.GetNext(pos);
			pMtObj->GetView (pObj);
...
// Object: View
// ************
IMPLEMENT_SERIAL (CmtView, CObject, 1)
CmtSessionManager* CmtView::GetSessionManager()
{
...
CmtSessionManager* CmtFolderView::GetSessionManager()
{
	return m_pBackParent? m_pBackParent->GetSessionManager(): NULL;
}

CmtView* CmtFolderView::SearchObject(CaLLQueryInfo* pInfo, CaView* pObj, BOOL b2QueryIfNotDone)
{
	...
	while (pos != NULL)
	{
		CmtView* pMtObj = m_listObject.GetNext (pos);
		CaView& obj = pMtObj->GetView();
		if (obj.Matched(pObj))
		{
			pMtObj->Access();
			return pMtObj;
...
HRESULT CmtFolderView::GetListObject(CaLLQueryInfo* pInfo, IStream*& pStream, BOOL bCopy2Stream)
{
	HRESULT hError = NOERROR;
	pStream = NULL;
	//
	...
	if (GetLastQuery() != -1)
	{
		...
		mReadWrite.UnLock();
		if (bCopy2Stream)
			hError = CopyViews2Stream (m_listObject, pStream);
		return hError;
	}
	...
	while (!lNew.IsEmpty())
	{
		...
		CmtView* pObjExist = SearchObject(pInfo, pObj, FALSE);
		...
		CmtView* pNewMtObject = new CmtView (pObj);
