...
/*
**    Source   : comprofi.cpp: implementation of the CmtProfile, CmtFolderProfile class
...
//                                 LOCAL FUNCTIONS
// ************************************************************************************************
static HRESULT CopyProfiles2Stream(CTypedPtrList< CObList, CmtProfile* >& listObject, IStream*& pStream)
{
	...
	try
	{
		...
		CTypedPtrList <CObList, CaProfile*> listObject2Store;
		//
		...
		while (pos != NULL)
		{
			CmtProfile* pMtObj = listObject.GetNext(pos);
			pMtObj->GetProfile (pObj);
...
// Object: Profile
// ***************
CmtSessionManager* CmtProfile::GetSessionManager()
{
...
// Folder of Profile:
// ******************
CmtFolderProfile::~CmtFolderProfile()
{
...
CmtSessionManager* CmtFolderProfile::GetSessionManager()
{
	return m_pBackParent? m_pBackParent->GetSessionManager(): NULL;
}

CmtProfile* CmtFolderProfile::SearchObject(CaLLQueryInfo* pInfo, CaProfile* pObj, BOOL b2QueryIfNotDone)
{
	...
	while (pos != NULL)
	{
		CmtProfile* pMtObj = m_listObject.GetNext (pos);
		CaProfile& obj = pMtObj->GetProfile();
		if (obj.Matched(pObj))
		{
			pMtObj->Access();
			return pMtObj;
...
HRESULT CmtFolderProfile::GetListObject(CaLLQueryInfo* pInfo, IStream*& pStream, BOOL bCopy2Stream)
{
	HRESULT hError = NOERROR;
	pStream = NULL;
	//
	...
	if (GetLastQuery() != -1)
	{
		...
		mReadWrite.UnLock();
		if (bCopy2Stream)
			hError = CopyProfiles2Stream (m_listObject, pStream);
		return hError;
	}
	...
	pInfo->SetObjectType(OBT_PROFILE);
	...
	while (!lNew.IsEmpty())
	{
		CaProfile* pObj = (CaProfile*)lNew.RemoveHead();
		CmtProfile* pObjExist = SearchObject(pInfo, pObj, FALSE);
		...
		CmtProfile* pNewMtObject = new CmtProfile (pObj);
