...
/*
**    Source   : comdbeve.cpp: implementation of the CmtDBEvent, CmtFolderDBEvent class
...
//                                 LOCAL FUNCTIONS
// ************************************************************************************************
static HRESULT CopyDBEvents2Stream(CTypedPtrList< CObList, CmtDBEvent* >& listObject, IStream*& pStream)
{
	...
	try
	{
		...
		CTypedPtrList <CObList, CaDBEvent*> listObject2Store;
		//
		...
		while (pos != NULL)
		{
			CmtDBEvent* pMtObj = listObject.GetNext(pos);
			pMtObj->GetDBEvent (pObj);
...
// Object: Procedure
// *****************
CmtSessionManager* CmtDBEvent::GetSessionManager()
{
...
// Folder of Procedure:
// ********************
CmtFolderDBEvent::~CmtFolderDBEvent()
{
...
CmtSessionManager* CmtFolderDBEvent::GetSessionManager()
{
	return m_pBackParent? m_pBackParent->GetSessionManager(): NULL;
}

CmtDBEvent* CmtFolderDBEvent::SearchObject(CaLLQueryInfo* pInfo, CaDBEvent* pObj, BOOL b2QueryIfNotDone)
{
	...
	while (pos != NULL)
	{
		CmtDBEvent* pMtObj = m_listObject.GetNext (pos);
		CaDBEvent& obj = pMtObj->GetDBEvent();
		if (obj.Matched(pObj))
		{
			pMtObj->Access();
			return pMtObj;
...
HRESULT CmtFolderDBEvent::GetListObject(CaLLQueryInfo* pInfo, IStream*& pStream, BOOL bCopy2Stream)
{
	HRESULT hError = NOERROR;
	pStream = NULL;
	//
	...
	if (GetLastQuery() != -1)
	{
		...
		mReadWrite.UnLock();
		if (bCopy2Stream)
			hError = CopyDBEvents2Stream (m_listObject, pStream);
		return hError;
	}
	...
	pInfo->SetObjectType(OBT_DBEVENT);
	...
	while (!lNew.IsEmpty())
	{
		...
		CmtDBEvent* pObjExist = SearchObject(pInfo, pObj, FALSE);
		...
		CmtDBEvent* pNewMtObject = new CmtDBEvent (pObj);
