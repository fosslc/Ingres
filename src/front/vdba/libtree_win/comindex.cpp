...
/*
**    Source   : comindex.cpp: implementation of the CmtIndex, CmtFolderIndex class
...
//                                 LOCAL FUNCTIONS
// ************************************************************************************************
static HRESULT CopyIndexes2Stream(CTypedPtrList< CObList, CmtIndex* >& listObject, IStream*& pStream)
{
	...
	try
	{
		...
		CTypedPtrList <CObList, CaIndex*> listObject2Store;
		//
		...
		while (pos != NULL)
		{
			CmtIndex* pMtObj = listObject.GetNext(pos);
			pMtObj->GetIndex (pObj);
...
// Object: Index
// *****************
CmtSessionManager* CmtIndex::GetSessionManager()
{
...
CmtSessionManager* CmtFolderIndex::GetSessionManager()
{
	return m_pBackParent? m_pBackParent->GetSessionManager(): NULL;
}

CmtIndex* CmtFolderIndex::SearchObject(CaLLQueryInfo* pInfo, CaIndex* pObj, BOOL b2QueryIfNotDone)
{
	...
	while (pos != NULL)
	{
		CmtIndex* pMtObj = m_listObject.GetNext (pos);
		CaIndex& obj = pMtObj->GetIndex();
		if (obj.Matched(pObj))
		{
			pMtObj->Access();
			return pMtObj;
...
HRESULT CmtFolderIndex::GetListObject(CaLLQueryInfo* pInfo, IStream*& pStream, BOOL bCopy2Stream)
{
	HRESULT hError = NOERROR;
	pStream = NULL;
	//
	...
	if (GetLastQuery() != -1)
	{
		...
		mReadWrite.UnLock();
		if (bCopy2Stream)
			hError = CopyIndexes2Stream (m_listObject, pStream);
		return hError;
	}
	...
	while (!lNew.IsEmpty())
	{
		CaIndex* pObj = (CaIndex*)lNew.RemoveHead();
		CmtIndex* pObjExist = SearchObject(pInfo, pObj, FALSE);
		...
		CmtIndex* pNewMtObject = new CmtIndex (pObj);
