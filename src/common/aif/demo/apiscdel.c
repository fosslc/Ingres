...
**
** Command syntax: apiscdel database_name
*/
...
static	char	createTBLText[] =
	"create table api_demo_cdel( name char(20) not null, age i4 not null )";
static	char	prepText[] =
	"prepare s0 from insert into api_demo_cdel values( ?, ? )";
static	char	execText[] = "execute s0";
static	char	openText[] = "select * from api_demo_cdel for update";
static	char	deleteText[] = "delete from api_demo_cdel ";
...
main( int argc, char** argv ) 
{
    ...
    if ( argc != 2 )
    {
	printf( "usage: apiscdel [vnode::]dbname[/server_class]\n" );
	exit( 0 );
    }
	...
	**  Delete using cursor
	*/
	printf( "apiscdel: delete row\n");

	queryParm.qy_genParm.gp_callback = NULL;
	queryParm.qy_genParm.gp_closure = NULL;
	queryParm.qy_connHandle = connHandle;
	queryParm.qy_queryType = IIAPI_QT_CURSOR_DELETE;
	queryParm.qy_queryText = deleteText;
