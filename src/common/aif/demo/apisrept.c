...
**
** Command syntax: apisrept database_name
**
*/
...
static	char	insertText[] =
	"insert into api_demo_rept values( $0 = ~V , $1 = ~V )";
...
main( int argc, char** argv ) 
{
    ...
    if ( argc != 2 )
    {
	printf( "usage: apisrept [vnode::]dbname[/server_class]\n" );
	exit( 0 );
    }
    ...
    */
    printf( "apisrept: define repeated insert\n" );

    queryParm.qy_genParm.gp_callback = NULL;
    queryParm.qy_genParm.gp_closure = NULL;
    queryParm.qy_connHandle = connHandle;
    queryParm.qy_queryType = IIAPI_QT_DEF_REPEAT_QUERY;
    queryParm.qy_queryText = insertText;
    ...
    */
    ...
    putParmParm.pp_parmData[1].dv_value = &insTBLInfo[0].age;
    ...
    */
	...
	reptHandle = getQInfoParm.gq_repeatQueryHandle;
	...
	**  Execute (repeated) insert.
	*/
    	printf( "apisrept: execute repeated insert\n" ); 
	...
	*/
    	...
    	putParmParm.pp_genParm.gp_closure = NULL;
    	putParmParm.pp_stmtHandle = stmtHandle;
    	putParmParm.pp_parmCount = 3;
    	putParmParm.pp_parmData = DataBuffer;
    	putParmParm.pp_moreSegments = 0;

    	putParmParm.pp_parmData[0].dv_null = FALSE;
    	putParmParm.pp_parmData[0].dv_length = sizeof( II_PTR );
    	putParmParm.pp_parmData[0].dv_value  = &reptHandle;
