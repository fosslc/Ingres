...
**	Commit transaction
**
** Command syntax: apiscomm database_name
*/
...
static	char queryText[] = "drop table api_demo_comm";
...
main( int argc, char** argv ) 
{
    ...
    if ( argc != 2 )
    {
	printf( "usage: apiscomm [vnode::]dbname[/server_class]\n" );
	exit( 0 );
    }

    IIdemo_init(); 
    IIdemo_conn(argv[1],&connHandle);
    IIdemo_query(&connHandle,&tranHandle,"drop table",queryText); 

    /*
    **  Commit transaction 
    */
    printf( "apiscomm: commiting a transaction\n");
