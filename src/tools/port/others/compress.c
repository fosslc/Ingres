...
**	20-jun-89 (arana)
**	    Put back ARGVAL macro removed with last change.
 *      08-jul-91 (johnr)
 *          Placed hp9000s300 specific change to fix signal def to void
 *	31-nov-92 (smc)
 *	    Repeated the hp9000s300 specific change for axp_osf: signal
 *	    handlers are void (*)() not int (*)().
 *	01-feb-93 (smc)
 *	    Tidied up signal handler functions to be TYPESIG (*)().
 *	28-feb-94 (vijay)
 *	    Remove extern defns.
 *	11-mar-94 (peterk)
 *	    Define rindex() only if not already provided
 *	27-apr-94 (andrewc)
 *	    Add prototype for rindex() if needed
 *	6-may-94 (andrewc)
 *	    Move declaration of rindex() after the inclusion of
 *	    generic.h
...
**	    Added stdlib.h
 *      17-May-1999 (hweho01) 
 *          Included <stdlib.h> file, it contains the prototype of malloc()
 *          function.
 */
 ...
 *
 * $Header: /m/roach/fe4/user/sandyd/projects/ingres65_fixup/ingres65/unix/tools/port/others/RCS/compress.c,v 1.1 92/04/17 10:31:05 sandyd Exp $
 * $Log:	compress.c,v $
 * Revision 1.1  92/04/17  10:31:05  sandyd
 * .
 * 
 * Revision 1.2  91/07/08  16:51:04  source
 * sc
 * 
 * Revision 1.1  90/03/09  09:18:05  source
 * Initialized as new code.
 * 
 * Revision 1.1  89/08/01  12:35:52  source
 * sc
 * 
 * Revision 1.3  89/06/20  15:09:20  source
 * sc
 * 
 * Revision 1.2  89/06/09  03:07:46  source
 * sc
 * 
 * Revision 1.1  87/07/30  18:48:48  roger
 * This is the new stuff.  We go dancing in.
 * 
 * Revision 1.1  86/12/04  11:24:23  source
 * sc
 * 
 ...
 *
 */
static char rcs_ident[] = "$Header: /m/roach/fe4/user/sandyd/projects/ingres65_fixup/ingres65/unix/tools/port/others/RCS/compress.c,v 1.1 92/04/17 10:31:05 sandyd Exp $";
...
char *ifname, *ofname;
{
    ...
    int mode;
#if DGUX
    static struct utimbuf ztimep;		/* guaranteed init'ed to zero */
    struct utimbuf timep = ztimep;
#else
    time_t timep[2];
#endif
...
TYPESIG oops ( )        /* wild pointer -- assume bad input */
{
    if ( do_decomp ) 
